...
IMPLEMENT_DYNAMIC(CSDOPCashSet, CDialog)
CSDOPCashSet::CSDOPCashSet(CWnd* pParent /*=NULL*/)
	: CDialog(CSDOPCashSet::IDD, pParent)
{
}

CSDOPCashSet::~CSDOPCashSet()
{
}

void CSDOPCashSet::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CSDOPCashSet, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// 初始化对话框
BOOL CSDOPCashSet::OnInitDialog()
{
...
void CSDOPCashSet::InitUI()
{
	...
	SetDlgItemText(IDC_LBLCURPCASH,theApp.operSDO.ReadValue("MSDO","CurPCash"));
	SetDlgItemText(IDC_LBLNEWPCASH,theApp.operSDO.ReadValue("MSDO","NewPCash"));

	SetDlgItemText(IDC_BTNOK,theApp.operSDO.ReadValue("MSDO","Makesure"));
	SetDlgItemText(IDC_BTNRESET,theApp.operSDO.ReadValue("MSDO","Close"));

	tbcResult.InsertItem(0,theApp.operSDO.ReadValue("MSDO","UIC_UI_tpgCharacterInfo"));//角色信息
	tbcResult.InsertItem(1,theApp.operSDO.ReadValue("MSDO","PS_UI_tpgPCash"));//修改红利点数
}

// 自动改变大小
void CSDOPCashSet::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CSDOPCashSet::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CSDOPCashSet::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码
...
// 显示角色资料信息
void CSDOPCashSet::ShowUserInfo(void)
{
	...
	GetDlgItem(IDC_LBLCURPCASH)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_ETCURPCASH)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_LBLNEWPCASH)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_ETNEWPCASH)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BTNOK)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BTNRESET)->ShowWindow(SW_HIDE);
}

//显示修改红利信息
void CSDOPCashSet::ShowPCashSet()
{
	...
	GetDlgItem(IDC_LBLCURPCASH)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_ETCURPCASH)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_LBLNEWPCASH)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_ETNEWPCASH)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_BTNOK)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_BTNRESET)->ShowWindow(SW_SHOW);
}

//处理快捷键响应函数
BOOL CSDOPCashSet::PreTranslateMessage(MSG* pMsg)
{
...
//单击用户资料信息的列名进行排序
void CSDOPCashSet::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operSDO.SortListInfo(&listUser,pNMHDR,pResult);
}

// 单击用户列表保存当前行号
void CSDOPCashSet::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CSDOPCashSet::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
		{
			...
			if(nItem >= 0)
			{
				...
				CurPCash = atoi(theApp.operSDO.GetValueByColName(&listUser, nItem, theApp.operSDO.ReadValue("MSDO","SDO_PCash")));

			}
			SetDlgItemInt(IDC_ETCURPCASH,CurPCash);
		}
		else
		{
			nItem = 0;
...
//查询玩家资料信息
void CSDOPCashSet::OnBnClickedSearch()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
//切换翻页组合框
void CSDOPCashSet::OnCbnSelchangeCmbuipage()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{	
...
// 接收返回信息
LRESULT CSDOPCashSet::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::SDO_CHARACTERINFO_QUERY_RESP:
		...
		break;
	case CEnumCore::Message_Tag_ID::SDO_PCash_Update_Resp:
		{
...
// 切换选项卡进行查询
void CSDOPCashSet::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
//修改玩家红利点数
void CSDOPCashSet::OnBnClickedBtnok()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
		...
		int nNewPCash=GetDlgItemInt(IDC_ETNEWPCASH);
		...
		_itoa(nNewPCash,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::SDO_PCash,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//


		int midlength = socketdata.EnBody();
		if(theApp.operSDO.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::SDO_PCash_Update))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operSDO.ReadIntValue("MSDO","Send");//当前的状态为发送查询请求
		}
...
//重置信息
void CSDOPCashSet::OnBnClickedBtnreset()
{
	// TODO: 在此添加控件通知处理程序代码
	SetDlgItemInt(IDC_ETNEWPCASH,0);
}
