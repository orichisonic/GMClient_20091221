...
IMPLEMENT_DYNAMIC(CSDOFamilyBaseDlg, CDialog)
CSDOFamilyBaseDlg::CSDOFamilyBaseDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CSDOFamilyBaseDlg::IDD, pParent)
{
	m_strPageSize = theApp.operSDO.ReadValue(INI_ERRORPROMPT_SECTION, "pageSize");
}

CSDOFamilyBaseDlg::~CSDOFamilyBaseDlg()
{
}

void CSDOFamilyBaseDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CSDOFamilyBaseDlg, CDialog)
...
// CSDOFamilyBaseDlg 消息处理程序
BOOL CSDOFamilyBaseDlg::OnInitDialog()
{
...
void CSDOFamilyBaseDlg::InitTableCtrl(TCB_ITEM tidArray[], DWORD dwCount, CString& strSection)
{
...
void CSDOFamilyBaseDlg::ResetCmbCtrl(CComboBox& cmbCtrl)
{
...
void CSDOFamilyBaseDlg::SetDataForListCtrl(CListCtrl &listCtrl, CSocketData& socketdata, CComboBox& cmbCtrl)
{
...
CString CSDOFamilyBaseDlg::GetDataFromListCtrl(CListCtrl &listCtrl, CEnumCore::TagName tagName, int nRowIndex)
{
	CHeaderCtrl* pListCtrlHeader = listCtrl.GetHeaderCtrl();
	ASSERT(NULL != pListCtrlHeader);
	int nListColNum = pListCtrlHeader->GetItemCount();
...
void CSDOFamilyBaseDlg::ClearTabCtrlData()
{
...
void CSDOFamilyBaseDlg::CreateCtrlEdit(CEdit& editCtrl, DWORD dwAboveCtrlID, DWORD dwEditID)
{
	RECT rct;
...
void CSDOFamilyBaseDlg::CreateCtrlStatic(CStatic& staticCtrl, DWORD dwAboveStaticCtrlID, DWORD dwStaticID)
{
	RECT rct;
...
void CSDOFamilyBaseDlg::CreateCtrlComCmb(CComboBox& cmbCtrl, DWORD dwCmbID, DWORD dwAboveCtrlID)
{	
/*	RECT rct;
	DWORD dwStyle;
	DWORD dwExStyle;
...
void CSDOFamilyBaseDlg::CreateCtrlList(CListCtrl& listCtrl, DWORD dwListID)
{
	RECT rctList;
...
void CSDOFamilyBaseDlg::MoveImpButton(DWORD dwAboveCtrlID, DWORD dwButtonID)
{
...
//创建button
void CSDOFamilyBaseDlg::CreateCtrlButton(CButton& buttonCtrl, DWORD dwAboveCtrlID, DWORD dwButtonID)
{
...
//创建DataTimePicker控件
void CSDOFamilyBaseDlg::CreateCtrlDateTime(CDateTimeCtrl& datatimeCtrl, DWORD dwAboveCtrlID, DWORD dwDateTimeID)//CDateTimeCtrl
{
	RECT rct;
	CWnd* pWnd = GetDlgItem(dwAboveCtrlID);
	ASSERT(NULL != pWnd);
...
void CSDOFamilyBaseDlg::OnBnClickedBtnbaseimp()
{
	ImplementUpdateOperation(m_nCurrentSelIndex);
}

void CSDOFamilyBaseDlg::OnBnClickedOk()
{
	QueryOperation(m_nCurrentSelIndex);
}

//查询家族信息
void CSDOFamilyBaseDlg::QueryFamilyInfo(CString &curPage)
{
	int nTFLVNum = 1;
	...
	// Account.
	nTFLVNum++;
	CString strFamily = GetFamily();//GetAccount();
	...
	try
	{
		...
		socketdata.AddTFLV(CEnumCore::TagName::SDO_FamilyName, CEnumCore::TagFormat::TLV_STRING,
			strFamily.GetLength(), (unsigned char *)(LPCTSTR)strFamily);
		...
		theApp.operSDO.SendQuery((unsigned char *)socketdata.getDes(), nDataLen, CEnumCore::ServiceKey::SDO_Family_Info_QUERY);
...
//查询玩家的其他信息
void CSDOFamilyBaseDlg::QueryOtherInfo(CEnumCore::ServiceKey eServiceKey, CString& curPage, CListCtrl& listCtrl)
{

	CString strServerIP = GetGameServerIP();
	CString strServerName = GetGameServerName();
	CString strFamilyName = GetDataFromListCtrl(listCtrl, CEnumCore::TagName::SDO_FamilyName);
	CString strFamilyID = GetDataFromListCtrl(listCtrl, CEnumCore::TagName::SDO_FamilyID);
...
void CSDOFamilyBaseDlg::OnBnClickedCancel()
{
//	ClearTabCtrlData();
//	ClearDataForCtrl();
...
void CSDOFamilyBaseDlg::SetRecvData(unsigned char * lpRecvbuf, int nLen, CEnumCore::Message_Tag_ID msgTagID)
{
...
void CSDOFamilyBaseDlg::InitGameServerComboBox(CSocketData& socketdata)
{
...
void CSDOFamilyBaseDlg::OnTcnSelchangeTabbase(NMHDR *pNMHDR, LRESULT *pResult)
{
	TCITEM tcItem;
...
void CSDOFamilyBaseDlg::SetTabCtrlItemCurSel(int nCurSel)
{
...
int CSDOFamilyBaseDlg::GetTabCtrlItemCurSel()
{
...
////////////////////////////////////////
// 指定列表框彈出菜單.
void CSDOFamilyBaseDlg::PopupMenuForListCtrlRClick(CListCtrl& listCtrl, CMenu& menu, POINT& point)
{
...
CString CSDOFamilyBaseDlg::GetGameServerIP()
{
	// Game Server IP.
...
CString CSDOFamilyBaseDlg::GetGameServerName()
{
	CString strGameName;
...
CString CSDOFamilyBaseDlg::GetFamily()
{
	CString Family;
	GetDlgItem(IDC_EDITBASEFAMILY)->GetWindowText(Family);

	return Family;
}

CString CSDOFamilyBaseDlg::GetCmbCurSelText(CComboBox& cmbPage)
{
	CString strCurText;
...
void CSDOFamilyBaseDlg::OnClose()
{
...
HBRUSH CSDOFamilyBaseDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
