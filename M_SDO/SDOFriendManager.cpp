...
void CSDOFriendManager::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operSDO.ReadValue("MSDO","FM_UI_tpgFriendInfo"));//好友信息
}
...
void CSDOFriendManager::FriendInfo()
{
	...
	{		
		...
		if(theApp.operSDO.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::SDO_Friend_Query))//好友信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operSDO.ReadIntValue("MSDO","Send");//当前的状态为发送查询请求
		}
...
void CSDOFriendManager::OnCbnSelchangeCmbpage()
{
	...
	{	
		if(pageInfo)//如果可以翻页的话
		{
			...
			if(theApp.operSDO.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::SDO_Friend_Query))//发送查询玩家好友信息
			{
				::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
				dlgStatus = theApp.operSDO.ReadIntValue("MSDO","Send");//当前的状态为发送查询请求
...
void CSDOFriendManager::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
		if(listResult.GetItemCount() > 0)
		{
			...
			if(nSelect >= 0)
			{
				UserID= atoi(theApp.operSDO.GetValueByColName(&listResult, nSelect, theApp.operSDO.ReadValue("MSDO","SDO_UserIndexID")));//玩家ID
				UserName = theApp.operSDO.GetValueByColName(&listUser, nItem, theApp.operSDO.ReadValue("MSDO","SDO_NickName"));//玩家昵称
