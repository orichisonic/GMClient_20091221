...
IMPLEMENT_DYNAMIC(CJW2CoupleCard, CDialog)
CJW2CoupleCard::CJW2CoupleCard(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2CoupleCard::IDD, pParent)
{
}

CJW2CoupleCard::~CJW2CoupleCard()
{
}

void CJW2CoupleCard::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2CoupleCard, CDialog)
...
// 初始化对话框
BOOL CJW2CoupleCard::OnInitDialog()
{
...
void CJW2CoupleCard::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","CC_UI_tpgCoupleCard"));

}

//处理快捷键响应函数
BOOL CJW2CoupleCard::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2CoupleCard::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2CoupleCard::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2CoupleCard::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2CoupleCard::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2CoupleCard::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2CoupleCard::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2CoupleCard::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
// 查询情侣信息
void CJW2CoupleCard::CoupleCard()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_CoupleParty_Card);
...
// 翻页进行查询
void CJW2CoupleCard::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			int midlength = socketdata.EnBody();				
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_CoupleParty_Card);// 查询玩家故事情节信息
...
// 接收返回信息
LRESULT CJW2CoupleCard::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2CoupleCard::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2CoupleCard::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2CoupleCard::OnBnClickedUicancel()
{
