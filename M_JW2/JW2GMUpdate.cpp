...
IMPLEMENT_DYNAMIC(CJW2GMUpdate, CDialog)
CJW2GMUpdate::CJW2GMUpdate(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2GMUpdate::IDD, pParent)
{
}

CJW2GMUpdate::~CJW2GMUpdate()
{
}

void CJW2GMUpdate::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2GMUpdate, CDialog)
...
// 初始化对话框
BOOL CJW2GMUpdate::OnInitDialog()
{
...
void CJW2GMUpdate::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","GU_UI_tpgGMUpdate"));
...
//处理快捷键响应函数
BOOL CJW2GMUpdate::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2GMUpdate::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2GMUpdate::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2GMUpdate::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2GMUpdate::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2GMUpdate::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2GMUpdate::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	CIni mIni;
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CJW2GMUpdate::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
	CIni mIni;
...
void CJW2GMUpdate::GMUpdate()
{
	...
	{		
		...
		int iStatus=cmbModifyType.GetCurSel();
		_itoa(iStatus,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::JW2_Status,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//操作员ID
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_GM_Update);
...
// 翻页进行查询
void CJW2GMUpdate::OnCbnSelchangeCmbuipage()
{
...
// 接收返回信息
LRESULT CJW2GMUpdate::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2GMUpdate::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2GMUpdate::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2GMUpdate::OnBnClickedUicancel()
{
...
//显示修改用户
void CJW2GMUpdate::ShowModifyUser(int n_cmdShow)
{
	...
	SetDlgItemText(IDC_LBL2,theApp.operJW2.ReadValue("MJW2","GU_UI_GMPOWER"));
...
void CJW2GMUpdate::InitModifyType(void)
{
	cmbModifyType.ResetContent();
	cmbModifyType.AddString(theApp.operJW2.ReadValue("MJW2","GU_UI_Normal"));
	cmbModifyType.AddString(theApp.operJW2.ReadValue("MJW2","GU_UI_Observe"));
	cmbModifyType.AddString(theApp.operJW2.ReadValue("MJW2","GU_UI_Admin"));
	cmbModifyType.SetCurSel(0);
}

void CJW2GMUpdate::OnBnClickedBtn1()
{
	// TODO: 在此添加控件通知处理程序代码
	GMUpdate();
}

void CJW2GMUpdate::OnBnClickedBtn2()
{
	InitModifyType();
}
