...
IMPLEMENT_DYNAMIC(CJW2PetInfo, CDialog)
CJW2PetInfo::CJW2PetInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2PetInfo::IDD, pParent)
{
}

CJW2PetInfo::~CJW2PetInfo()
{
}

void CJW2PetInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2PetInfo, CDialog)
...
void CJW2PetInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","PI_UI_tpgPetInfo"));


}

//处理快捷键响应函数
BOOL CJW2PetInfo::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2PetInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2PetInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2PetInfo::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2PetInfo::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2PetInfo::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2PetInfo::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2PetInfo::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CJW2PetInfo::PetInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_PetInfo_Query);
...
// 翻页进行查询
void CJW2PetInfo::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_PetInfo_Query);// 查询玩家故事情节信息
...
// 接收返回信息
LRESULT CJW2PetInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2PetInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2PetInfo::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2PetInfo::OnBnClickedUicancel()
{
