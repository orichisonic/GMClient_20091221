...
IMPLEMENT_DYNAMIC(CJW2ItemLog, CDialog)
CJW2ItemLog::CJW2ItemLog(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2ItemLog::IDD, pParent)
{
}

CJW2ItemLog::~CJW2ItemLog()
{
}

void CJW2ItemLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2ItemLog, CDialog)
...
void CJW2ItemLog::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","IL_UI_tpgItemLog"));


}

//处理快捷键响应函数
BOOL CJW2ItemLog::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2ItemLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2ItemLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2ItemLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2ItemLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2ItemLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2ItemLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2ItemLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CJW2ItemLog::ItemLog()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_Item_Log);
...
// 翻页进行查询
void CJW2ItemLog::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			int midlength = socketdata.EnBody();				
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_Item_Log);// 查询玩家故事情节信息
...
// 接收返回信息
LRESULT CJW2ItemLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2ItemLog::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2ItemLog::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2ItemLog::OnBnClickedUicancel()
{
