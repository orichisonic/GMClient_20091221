...
IMPLEMENT_DYNAMIC(CJW2BanList, CDialog)
CJW2BanList::CJW2BanList(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2BanList::IDD, pParent)
{
}

CJW2BanList::~CJW2BanList()
{
}

void CJW2BanList::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2BanList, CDialog)
...
void CJW2BanList::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","BU_UI_TpgList"));
...
//处理快捷键响应函数
BOOL CJW2BanList::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2BanList::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2BanList::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询资料信息
void CJW2BanList::OnBnClickedUisearch()
{
	try
	{
...
void CJW2BanList::UserInfo()
{
	...
	{		
		...
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_ACCOUNT_BANISHMENT_QUERY);//发送查询玩家资料信息的请求
...
// 查询所有被封停帐号列表信息
void CJW2BanList::AllBanUserList()
{
	try
	{				
		CIni mIni;
...
// 翻页进行查询
void CJW2BanList::OnCbnSelchangeCmbuipage()
{	
	try
	{		
		if(pageInfo)
...
// 翻页进行查询玩家资料信息
void CJW2BanList::OnCbnSelchangeCmbpage()
{
	try
	{		
		if(pageUInfo)
...
// 单击资料信息保存当前行号
void CJW2BanList::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击资料信息进行操作
void CJW2BanList::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listUser.GetItemCount() != 0)
...
// 单击封停账号列表信息保存当前行号
void CJW2BanList::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击封停账号列表信息进行解封
void CJW2BanList::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listResult.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CJW2BanList::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{	
	CIni mIni;
...
// 封停或解封帐号
void CJW2BanList::OnBnClickedBtnban()
{
	try
	{
		CIni mIni;
...
// 重置信息
void CJW2BanList::OnBnClickedBtnreset()
{
	SetDlgItemText(IDC_EDITBANREASON,"");
...
// 接收返回信息
LRESULT CJW2BanList::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//	case CEnumCore::Message_Tag_ID::JW2_ACCOUNT_QUERY_RESP://查询玩家资料信息的回应
	case CEnumCore::Message_Tag_ID::JW2_ACCOUNTREMOTE_QUERY_RESP://查询玩家资料信息的回应
...
//单击用户资料信息的列名进行排序
void CJW2BanList::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operJW2.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CJW2BanList::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operJW2.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CJW2BanList::OnBnClickedUicancel()
{
