...
IMPLEMENT_DYNAMIC(CJW2MMoney, CDialog)
CJW2MMoney::CJW2MMoney(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2MMoney::IDD, pParent)
{
}

CJW2MMoney::~CJW2MMoney()
{
}

void CJW2MMoney::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2MMoney, CDialog)
...
void CJW2MMoney::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","MM_UI_tpgMMoney"));
	tbcResult.InsertItem(2,theApp.operJW2.ReadValue("MJW2","MM_UI_tpgCashMoney"));
	ShowcmbBox(SW_HIDE);
...
//处理快捷键响应函数
BOOL CJW2MMoney::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2MMoney::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2MMoney::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2MMoney::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2MMoney::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2MMoney::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2MMoney::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2MMoney::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CJW2MMoney::CashMoneyLog()
{
	...
	{		
		...
		_itoa(iType,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::JW2_MoneyType, CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//类型
...
// 翻页进行查询
void CJW2MMoney::OnCbnSelchangeCmbuipage()
{
...
// 接收返回信息
LRESULT CJW2MMoney::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2MMoney::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2MMoney::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2MMoney::OnBnClickedUicancel()
{
...
bool CJW2MMoney::InitCmbType(void)
{
	m_cmbType.ResetContent();
	m_cmbType.AddString("喂养宠物");
	m_cmbType.AddString("进化宠物");
	m_cmbType.AddString("买动作");
	m_cmbType.AddString("信纸");
	m_cmbType.AddString("创建家族");
	m_cmbType.AddString("换肤色");
	m_cmbType.SetCurSel(0);
	return true;
}

bool CJW2MMoney::ShowcmbBox(int cmdShow)
{
...
void CJW2MMoney::OnCbnSelchangeCmbuitype()
{
	// TODO: 在此添加控件通知处理程序代码
	CashMoneyLog();
}
