...
IMPLEMENT_DYNAMIC(CJW2Messenger, CDialog)
CJW2Messenger::CJW2Messenger(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2Messenger::IDD, pParent)
{
}

CJW2Messenger::~CJW2Messenger()
{
}

void CJW2Messenger::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2Messenger, CDialog)
...
// 初始化对话框
BOOL CJW2Messenger::OnInitDialog()
{
...
void CJW2Messenger::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","MS_UI_tpgMessenger"));
}

//处理快捷键响应函数
BOOL CJW2Messenger::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2Messenger::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2Messenger::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2Messenger::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2Messenger::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2Messenger::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2Messenger::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2Messenger::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CJW2Messenger::Messenger()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_Messenger_Query);
...
// 翻页进行查询
void CJW2Messenger::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			int midlength = socketdata.EnBody();				
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_Messenger_Query);// 查询玩家故事情节信息
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operJW2.ReadIntValue("MJW2","Send");//当前的状态为发送查询请求
			socketdata.Destroy();
		}		
...
// 接收返回信息
LRESULT CJW2Messenger::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2Messenger::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2Messenger::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2Messenger::OnBnClickedUicancel()
{
