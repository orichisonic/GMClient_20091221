...
IMPLEMENT_DYNAMIC(CJW2MailInfo, CDialog)
CJW2MailInfo::CJW2MailInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2MailInfo::IDD, pParent)
{
}

CJW2MailInfo::~CJW2MailInfo()
{
}

void CJW2MailInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2MailInfo, CDialog)
...
// 初始化对话框
BOOL CJW2MailInfo::OnInitDialog()
{
...
void CJW2MailInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","MI_UI_tpgMailInfo"));
}

//处理快捷键响应函数
BOOL CJW2MailInfo::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2MailInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2MailInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2MailInfo::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2MailInfo::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2MailInfo::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2MailInfo::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2MailInfo::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CJW2MailInfo::MailInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_MailInfo_Query);
...
// 翻页进行查询
void CJW2MailInfo::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			int midlength = socketdata.EnBody();				
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_MailInfo_Query);// 查询玩家故事情节信息
...
// 接收返回信息
LRESULT CJW2MailInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2MailInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2MailInfo::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2MailInfo::OnBnClickedUicancel()
{
