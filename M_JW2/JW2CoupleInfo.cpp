...
IMPLEMENT_DYNAMIC(CJW2CoupleInfo, CDialog)
CJW2CoupleInfo::CJW2CoupleInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2CoupleInfo::IDD, pParent)
{
}

CJW2CoupleInfo::~CJW2CoupleInfo()
{
}

void CJW2CoupleInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2CoupleInfo, CDialog)
...
// 初始化对话框
BOOL CJW2CoupleInfo::OnInitDialog()
{
...
void CJW2CoupleInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","CI_UI_tpgCoupleInfo"));
	tbcResult.InsertItem(2,theApp.operJW2.ReadValue("MJW2","CI_UI_tpgCoupleLog"));

}

//处理快捷键响应函数
BOOL CJW2CoupleInfo::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2CoupleInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2CoupleInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2CoupleInfo::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CJW2CoupleInfo::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
		if(pageUInfo)//如果可以翻页的话
		{
			...
			int iPageSize = operJW2.ReadIntValue("MJW2","pageSize");//每页显示50条记录
			...
			int midlength = socketdata.EnBody();
			operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_ACCOUNT_QUERY);//发送查询玩家资料信息的请求

			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = operJW2.ReadIntValue("MJW2","Send");//当前的状态为发送查询请求
...
// 单击用户列表保存当前行号
void CJW2CoupleInfo::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2CoupleInfo::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2CoupleInfo::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
// 查询情侣信息
void CJW2CoupleInfo::CoupleInfo()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_COUPLEINFO_QUERY);
...
void CJW2CoupleInfo::CoupleLog()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_COUPLELOG_QUERY);
...
// 翻页进行查询
void CJW2CoupleInfo::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			socketdata.AddTFLV(CEnumCore::TagName::JW2_ServerIP,CEnumCore::TagFormat::TLV_STRING,strlen(ServerIP),(unsigned char *)mIni.wcharToChar(ServerIP));//服务器IP
			socketdata.AddTFLV(CEnumCore::TagName::JW2_UserID,CEnumCore::TagFormat::TLV_STRING,strlen(UserAccount),(unsigned char *)mIni.wcharToChar(UserAccount));//玩家帐号
			...
			if(tbcResult.GetCurSel() == 1)
			{
				theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_COUPLEINFO_QUERY);// 查询玩家故事情节信息
			}
			else if(tbcResult.GetCurSel() == 2)
			{
				theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_COUPLELOG_QUERY);// 查询玩家身上道具信息
			}
...
// 接收返回信息
LRESULT CJW2CoupleInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2CoupleInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2CoupleInfo::OnLvnColumnclickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2CoupleInfo::OnBnClickedUicancel()
{
