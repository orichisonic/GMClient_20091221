...
// CJW2FamilyFundLog 对话框

IMPLEMENT_DYNAMIC(CJW2FamilyFundLog, CDialog)
CJW2FamilyFundLog::CJW2FamilyFundLog(CWnd* pParent /*=NULL*/)
	: CDialog(CJW2FamilyFundLog::IDD, pParent)
{
}

CJW2FamilyFundLog::~CJW2FamilyFundLog()
{
}

void CJW2FamilyFundLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CJW2FamilyFundLog, CDialog)
...
// 初始化对话框
BOOL CJW2FamilyFundLog::OnInitDialog()
{
...
void CJW2FamilyFundLog::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operJW2.ReadValue("MJW2","FFL_UI_tpgDevoteLog"));
	tbcResult.InsertItem(2,theApp.operJW2.ReadValue("MJW2","FFL_UI_tpgConsumeLog"));

}

//处理快捷键响应函数
BOOL CJW2FamilyFundLog::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CJW2FamilyFundLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CJW2FamilyFundLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CJW2FamilyFundLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 查询玩家资料信息
void CJW2FamilyFundLog::FamilyInfo()
{	
...
// 翻页查询玩家资料信息
void CJW2FamilyFundLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CJW2FamilyFundLog::OnNMClickListfamily(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CJW2FamilyFundLog::OnNMDblclkListfamily(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listFamily.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CJW2FamilyFundLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
// 查询家族成员信息
void CJW2FamilyFundLog::FamilyFundLog()
{
	try
	{		
		CIni mIni;
		...
		int iStatus=tbcResult.GetCurSel()-1;
		_itoa(iStatus,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::JW2_Status,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//玩家ID
		...
		int midlength = socketdata.EnBody();
		theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_FamilyFund_Log);
...
// 翻页进行查询
void CJW2FamilyFundLog::OnCbnSelchangeCmbuipage()
{
	...
	try
	{
		if(pageInfo)
		{
			...
			int midlength = socketdata.EnBody();				
			theApp.operJW2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::JW2_FamilyFund_Log);// 查询玩家故事情节信息
...
// 接收返回信息
LRESULT CJW2FamilyFundLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
//单击用户资料信息的列名进行排序
void CJW2FamilyFundLog::OnLvnColumnclickListfamily(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CJW2FamilyFundLog::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 关闭对话框
void CJW2FamilyFundLog::OnBnClickedUicancel()
{
...
// 关闭对话框
void CJW2FamilyFundLog::OnClose()
{
