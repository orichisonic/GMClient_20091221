...
IMPLEMENT_DYNAMIC(CCG2PetPic, CDialog)
CCG2PetPic::CCG2PetPic(CWnd* pParent /*=NULL*/)
	: CDialog(CCG2PetPic::IDD, pParent)
{
}

CCG2PetPic::~CCG2PetPic()
{
}

void CCG2PetPic::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDITUSERSN, editUserSn);
	DDX_Control(pDX, IDC_CMBPETPICNAME, cmbPet);
}


BEGIN_MESSAGE_MAP(CCG2PetPic, CDialog)
...
void CCG2PetPic::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgPetPic"));
	tbcResult.InsertItem(2,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgAddPetPic"));
}


// 自动改变大小
void CCG2PetPic::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CCG2PetPic::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CCG2PetPic::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CCG2PetPic::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CCG2PetPic::OnNMClickListUser(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CCG2PetPic::OnNMDblclkListUser(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 单击宠物列表保存当前行号
void CCG2PetPic::OnNMClickListPet(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击宠物信息进行查询
void CCG2PetPic::OnNMDblclkListPet(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 1)
	{
		if(listPet.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CCG2PetPic::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CCG2PetPic::PetPic()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_PetPic_Query );
...
void CCG2PetPic::AddPetPic()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			PetID=1;
			_itoa(PetID,strInt,10);
			socketdata.AddTFLV(CEnumCore::TagName::CG2_TypeID,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//宠物ID
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_PetPic_Del_Query);
...
// 翻页进行查询
void CCG2PetPic::OnCbnSelchangeCmbuipage()
{
	//OperationCG2 theApp.operCG2;	
	try
	{
...
// 接收返回信息
LRESULT CCG2PetPic::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
	...
	case CEnumCore::Message_Tag_ID::CG2_Account_Query_Resp://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::CG2_PetPic_Query_Resp://查询宠物信息的回应
		{
...
// 关闭对话框
void CCG2PetPic::OnBnClickedUicancel()
{
...
//显示页面
void CCG2PetPic::ShowAddPetPic(int n_cmdShow)
{
	GetDlgItem(IDC_LBLSTATIC0)->ShowWindow(n_cmdShow);
	GetDlgItem(IDC_LBLSTATIC1)->ShowWindow(n_cmdShow);
	GetDlgItem(IDC_EDITUSERSN)->ShowWindow(n_cmdShow);
	GetDlgItem(IDC_CMBPETPICNAME)->ShowWindow(n_cmdShow);
	GetDlgItem(IDC_BTNCONFIRM)->ShowWindow(n_cmdShow);
	GetDlgItem(IDC_BTNRESET)->ShowWindow(n_cmdShow);

	SetDlgItemText(IDC_LBLSTATIC0,theApp.operCG2.ReadValue("MCG2","UIC_CharNick"));
	SetDlgItemText(IDC_LBLSTATIC1,theApp.operCG2.ReadValue("MCG2","BU_UI_PETPICNAME"));
	SetDlgItemText(IDC_BTNCONFIRM,theApp.operCG2.ReadValue("MCG2","BU_UI_ADDPETPIC"));
	SetDlgItemText(IDC_BTNRESET,theApp.operCG2.ReadValue("MCG2","BU_UI_RESET"));

	if((listUser.GetItemCount() > 0) && (nItem>=0))
	{
		CString UserID=theApp.operCG2.GetValueByColName(&listUser, nItem, theApp.operCG2.ReadValue("MCG2","CG2_UserID"));
		SetDlgItemText(IDC_EDITUSERSN,UserID);
		editUserSn.SetReadOnly(true);

	}
}


void CCG2PetPic::OnBnClickedBtnconfirm()
{
	// TODO: 在此添加控件通知处理程序代码
	AddPetPic();
}
