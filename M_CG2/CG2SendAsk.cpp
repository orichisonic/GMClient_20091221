...
IMPLEMENT_DYNAMIC(CCG2SendAsk, CDialog)
CCG2SendAsk::CCG2SendAsk(CWnd* pParent /*=NULL*/)
	: CDialog(CCG2SendAsk::IDD, pParent)
{
}

CCG2SendAsk::~CCG2SendAsk()
{
}

void CCG2SendAsk::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCG2SendAsk, CDialog)
...
void CCG2SendAsk::InitUI()
{
	...
	SetDlgItemText(IDC_LBLHINT,theApp.operCG2.ReadValue("MCG2","BU_UI_SendAsk"));	

}

// 自动改变大小
void CCG2SendAsk::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CCG2SendAsk::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CCG2SendAsk::OnBnClickedUisearch()
{
	try
	{
		//OperationCG2 theApp.operCG2;
...
// 翻页进行查询
void CCG2SendAsk::OnCbnSelchangeCmbkppage()
{	
	try
	{
...
// 双击玩家资料信息进行踢人
void CCG2SendAsk::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listResult.GetItemCount() != 0)
		{
			...
			{	
				...
				if (AfxMessageBox("确认要向用户发送问答信息", MB_YESNO) == IDNO) 
				{
					return;
				}
				...
				int midlength = socketdata.EnBody();
				theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_SendAsk_Query);//发送强制玩家下线的请求
...
// 接收返回信息
LRESULT CCG2SendAsk::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::CG2_Account_Query_Resp ://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::CG2_SendAsk_Query_Resp://强制玩家下线的回应
		{
...
// 关闭对话框
void CCG2SendAsk::OnBnClickedUicancel()
{
