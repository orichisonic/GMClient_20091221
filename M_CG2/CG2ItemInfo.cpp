...
IMPLEMENT_DYNAMIC(CCG2ItemInfo, CDialog)
CCG2ItemInfo::CCG2ItemInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CCG2ItemInfo::IDD, pParent)
{
}

CCG2ItemInfo::~CCG2ItemInfo()
{
}

void CCG2ItemInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCG2ItemInfo, CDialog)
...
// 初始化对话框
BOOL CCG2ItemInfo::OnInitDialog()
{
...
void CCG2ItemInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgPackItem"));
	tbcResult.InsertItem(2,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgTempItem"));
	tbcResult.InsertItem(3,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgCharItem"));
	tbcResult.InsertItem(4,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgShopItem"));

}


// 自动改变大小
void CCG2ItemInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CCG2ItemInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CCG2ItemInfo::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CCG2ItemInfo::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CCG2ItemInfo::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CCG2ItemInfo::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CCG2ItemInfo::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
	//OperationCG2 theApp.operCG2;
...
void CCG2ItemInfo::PackItem()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_PackItem_Query );
...
void CCG2ItemInfo::TempItem()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_TempItem_Query);
...
void CCG2ItemInfo::CharItem()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_CharItem_Query);
...
void CCG2ItemInfo::ShopItem()
{
	...
	try
	{
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_ShopItem_Query);
...
// 翻页进行查询
void CCG2ItemInfo::OnCbnSelchangeCmbuipage()
{
	//OperationCG2 theApp.operCG2;	
	try
	{
		if(pageInfo)
		{
			...
			if((listUser.GetItemCount() > 0) && (nItem>=0))
			{
				...
				if(tbcResult.GetCurSel() == 1)
				{
					theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_PackItem_Query);// 查询角色详细资料
				}
				else if(tbcResult.GetCurSel() == 2)
				{
					theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_TempItem_Query);// 查询角色属性资料
				}
				else if(tbcResult.GetCurSel() == 3)
				{
					theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_CharItem_Query);// 查询角色工会资料
				}
				else if(tbcResult.GetCurSel() == 4)
				{
					theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_ShopItem_Query );// 查询角色称号资料
				}
...
// 接收返回信息
LRESULT CCG2ItemInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 关闭对话框
void CCG2ItemInfo::OnBnClickedUicancel()
{
