...
IMPLEMENT_DYNAMIC(CCG2BakAccount, CDialog)
CCG2BakAccount::CCG2BakAccount(CWnd* pParent /*=NULL*/)
	: CDialog(CCG2BakAccount::IDD, pParent)
{
}

CCG2BakAccount::~CCG2BakAccount()
{
}

void CCG2BakAccount::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCG2BakAccount, CDialog)
...
// 初始化对话框
BOOL CCG2BakAccount::OnInitDialog()
{
...
void CCG2BakAccount::InitUI()
{
	...
	SetDlgItemText(IDC_LBLHINT,"提示:双击账号,可以恢复角色");
}

// 自动改变大小
void CCG2BakAccount::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CCG2BakAccount::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CCG2BakAccount::OnBnClickedUisearch()
{
	try
	{
		//OperationCG2 theApp.operCG2;
...
// 翻页进行查询
void CCG2BakAccount::OnCbnSelchangeCmbkppage()
{	
	try
	{
...
// 双击玩家资料信息进行恢复角色
void CCG2BakAccount::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listResult.GetItemCount() != 0)
		{
			...
			{	
				...
				if (AfxMessageBox("确实要恢复该角色吗?", MB_YESNO) == IDNO) 
				{
					return;
				}
				...
				int midlength = socketdata.EnBody();
				theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_Recovery_Account_Qurey);//发送强制玩家下线的请求
...
// 接收返回信息
LRESULT CCG2BakAccount::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::CG2_BakAccount_Query_Resp ://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::CG2_Recovery_Account_Qurey_Resp://恢复角色回应
		{
...
// 重置对话框
void CCG2BakAccount::OnBnClickedUicancel()
{
...
BOOL CCG2BakAccount::PreTranslateMessage(MSG* pMsg)
{
		...
		if(GetKeyState(VK_CONTROL)&0x8000)
		{
			...
			{				
				if(pMsg->wParam == 'C')
				{
					...
					{
						theApp.operCG2.CopyListInfo(&listResult);				
					}
				}
