...
// CCG2ChangeCharNo 对话框

IMPLEMENT_DYNAMIC(CCG2ChangeCharNo, CDialog)
CCG2ChangeCharNo::CCG2ChangeCharNo(CWnd* pParent /*=NULL*/)
	: CDialog(CCG2ChangeCharNo::IDD, pParent)
{
}

CCG2ChangeCharNo::~CCG2ChangeCharNo()
{
}

void CCG2ChangeCharNo::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CMBNEWCHARNO, cmbNewCharNo);
}


BEGIN_MESSAGE_MAP(CCG2ChangeCharNo, CDialog)
	ON_WM_CLOSE()
...
// CCG2ChangeCharNo 消息处理程序

BOOL CCG2ChangeCharNo::OnInitDialog()
{
...
void CCG2ChangeCharNo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operCG2.ReadValue("MCG2","BU_UI_tpgChangeCharNo"));//角色换位

	GetDlgItem(IDC_STATICACCOUNT)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STATICCURCHARNO)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_ETACCOUNT)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_ETCURCHARNO)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STATICNEWCHARNO)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_CMBNEWCHARNO)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BTUPDATE)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BTRESET)->ShowWindow(SW_HIDE);
}

//关闭对话框
void CCG2ChangeCharNo::OnClose()
{
...
void CCG2ChangeCharNo::OnBnClickedUicancel()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CCG2ChangeCharNo::OnSize(UINT nType, int cx, int cy)
{
...
HBRUSH CCG2ChangeCharNo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CCG2ChangeCharNo::UserInfo()
{
	//OperationCG2 theApp.operCG2;
	try
	{		
...
void CCG2ChangeCharNo::OnBnClickedUisearch()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
void CCG2ChangeCharNo::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	//OperationCG2 theApp.operCG2;
	...
	else if(tbcResult.GetCurSel()==1)
	{
		...
		GetDlgItem(IDC_STATICCURCHARNO)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_ETACCOUNT)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_ETCURCHARNO)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_STATICNEWCHARNO)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_CMBNEWCHARNO)->ShowWindow(SW_SHOW);
		...
		if((listUser.GetItemCount() > 0) && (nItem >= 0))
		{
			...
			SetDlgItemInt(IDC_ETCURCHARNO,iCharNo);
		}
		else
		{
			cmbNewCharNo.ResetContent();
		}
...
// 单击用户列表保存当前行号
void CCG2ChangeCharNo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CCG2ChangeCharNo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	//OperationCG2 theApp.operCG2;
...
//修改玩家角色职业
void CCG2ChangeCharNo::OnBnClickedBtupdate()
{
	// TODO: 在此添加控件通知处理程序代码
	//OperationCG2 theApp.operCG2;
	try
	{
		...
		iNewCharNo=GetDlgItemInt(IDC_CMBNEWCHARNO);
		...
		_itoa(iNewCharNo,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::CG2_ChangeCharNo,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//页数


		int midlength = socketdata.EnBody();
		theApp.operCG2.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::CG2_ChangeCharNo_Query);
...
//
void CCG2ChangeCharNo::OnBnClickedBtreset()
{
	// TODO: 在此添加控件通知处理程序代码
...
LRESULT CCG2ChangeCharNo::OnMessageReceive(WPARAM wParam,LPARAM lParam)
{
...
void CCG2ChangeCharNo::InitCmbCharNo(void)
{
	cmbNewCharNo.ResetContent();
	cmbNewCharNo.AddString("0");
	cmbNewCharNo.AddString("1");
	cmbNewCharNo.AddString("2");
	cmbNewCharNo.AddString("3");
	cmbNewCharNo.AddString("4");
	cmbNewCharNo.AddString("5");
	cmbNewCharNo.SetCurSel(0);

}

BOOL CCG2ChangeCharNo::PreTranslateMessage(MSG* pMsg)
{
