...
LPCTSTR INI_GAME_DB_SECTION = "GAMEDB";
LPCTSTR INI_TAG_NAME_SECTION = "TAGNAME";
LPCTSTR INI_ERROR_PROMPT_SECTION = "ERRORPROMPT";
LPCTSTR INI_UI_TEXT_SECTION = "UITEXT";

#define IDC_BANTIMEHINT 5022

// ID of ctrl with text.
DWORD g_dwCtrlIDWithText[] = 
{
	// Static contrl.
	IDC_STATICUBANTIME,
	IDC_STATICBANTIME,
...
GAME_OPER_INFO g_sGameOperInfo[] = 
{
///////////////////////////////////////////////////
...
DWORD g_dwTabCtrlIDMapCtrlWndID[][MAX_TCN_ASSO_WND] = 
{
...
BEGIN_MESSAGE_MAP(CAccountDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_GAMENAME, &CAccountDlg::OnCbnSelChangeGameName)
	ON_CBN_SELCHANGE(IDC_CMBPAGE, &CAccountDlg::OnCbnSelChangePage)
	ON_BN_CLICKED(IDCANCEL, &CAccountDlg::OnBnClickedCancel)
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB, &CAccountDlg::OnTcnSelchangeTab)
	ON_BN_CLICKED(IDC_BTNIMPLEMENT, &CAccountDlg::OnBnClickedImplement)
	//maple add
	ON_BN_CLICKED(IDC_OPTIONUBANTIME,&CAccountDlg::OnBnClickBanTimeOption)
END_MESSAGE_MAP()
...
BOOL CAccountDlg::OnInitDialog()
{
	...
	int nIndex =0;
	// Set ctrl text.
	for (i=0; i<sizeof(g_dwCtrlIDWithText)/sizeof(DWORD); i++)
	{
		if (i != IDC_TAB)
		{
			char szKey[MAX_PATH] = {0};
			_itoa(g_dwCtrlIDWithText[i], szKey, 10);
			CString strText = theApp.m_iniFile.ReadStringValue(INI_UI_TEXT_SECTION, szKey);
			CWnd* pCtrl = GetDlgItem(g_dwCtrlIDWithText[i]);
			ASSERT(NULL != pCtrl);
	...
	for (i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
	{
	...
	for (i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
	{
		...
		if(iGameID!=0)
		{
			LPGAME_INFO lpgi = new GAME_INFO;
...
void CAccountDlg::InitTabCtrl()
{
	...
	//	sprintf_s(szKey, "%d_%d", IDC_TAB, i);
		...
		CString strTxt = theApp.m_iniFile.ReadStringValue(INI_UI_TEXT_SECTION, szKey);
...
void CAccountDlg::LayoutChildWnd()
{
	RECT rctDlg;
...
void CAccountDlg::SetGameDBServerComboBox(LPGAME_INFO lpgi)
{
	ASSERT(NULL != lpgi);
	...
	try
	{
		...
		int nIndex = 0;
		LPGAME_DB_SERVER lpgds = lpgi->lplistDBServer;
		while(NULL != lpgds)
		{
...
void CAccountDlg::QueryGameName()
{
...
void CAccountDlg::QueryGameServerIp(int nGameID, int nGameDBID)
{
	try
	{
		CSocketData socketdata;
...
void CAccountDlg::SetRecvData(CSocketData &socketdata, CEnumCore::Message_Tag_ID eMsgTagID)
{
	EnableWindow(TRUE);
		...
		case CEnumCore::Message_Tag_ID::SERVERINFO_IP_QUERY_RESP:	// 游戏DB列表.
			{
				...
				ASSERT(NULL != pvgi);
				LPGAME_INFO lpgi = (LPGAME_INFO)pvgi;

				// make GameDB list.
				int nTFLVNum = socketdata.getAllTFLV();
				LPGAME_DB_SERVER lpLastDBServer = NULL;
				for (int i=1; i<nTFLVNum; i++)
				{
					...
					if ((unsigned short)tflvServerCity.m_tagName == CEnumCore::TagName::ServerInfo_City)
					{
						...
						{
							lpgi->lplistDBServer = new GAME_DB_SERVER;
							lpLastDBServer = lpgi->lplistDBServer;						
						}
						else
						{
							lpLastDBServer->lpNext = new GAME_DB_SERVER;
		...
		case CEnumCore::Message_Tag_ID::Ban_Account_Query_Resp:		// 查询单个帐号的封停信息回应.
			{
				...
				if (nTFLVColNum-1 != m_listColumnNum)
				{
					...
					for (int i=0; i<m_listColumnNum; i++)
					{
						...
						CString strTitle = theApp.m_iniFile.ReadStringValue(INI_TAG_NAME_SECTION, (LPCTSTR)strKey);
...
void CAccountDlg::OnBnClickedOk()
{
	...
	// Get game id.
	nTFLVNum++;
	LPGAME_INFO lpgi = (LPGAME_INFO)m_cmbGameName.GetItemDataPtr(nSel);
	ASSERT(NULL != lpgi);
...
void CAccountDlg::OnCbnSelChangeGameName()
{
	// Get gameDBID-Key.
	int nIndex = m_cmbGameName.GetCurSel();
	LPGAME_INFO lpgi = (LPGAME_INFO)m_cmbGameName.GetItemData(nIndex);
	ASSERT(NULL != lpgi);

	if (NULL == lpgi->lplistDBServer)
	{
		CString strKey("");
		for (int i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
		{
		...
		// Get GameDBID.
		int nGameDBID = theApp.m_iniFile.ReadIntegrityValue(INI_GAME_DB_SECTION, strKey);
...
void CAccountDlg::OnCbnSelChangePage()
{
	pageInfo=false;
	int nSel = 0;
...
void CAccountDlg::OnTcnSelchangeTab(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		if (0 == g_dwTabCtrlIDMapCtrlWndID[m_nTabCtrlSelectItem][i])
			continue;

		CWnd* pCtrlHide = GetDlgItem(g_dwTabCtrlIDMapCtrlWndID[m_nTabCtrlSelectItem][i]);
	...
	for (int i=0; i<MAX_TCN_ASSO_WND; i++)
	{
		if (0 == g_dwTabCtrlIDMapCtrlWndID[nSel][i])
			continue;

		ASSERT(nSel < MAX_TCN_ASSO_WND);
		CWnd* pCtrlHide = GetDlgItem(g_dwTabCtrlIDMapCtrlWndID[nSel][i]);
		ASSERT(NULL != pCtrlHide);
...
void CAccountDlg::OnBnClickedImplement()
{
	int nSel = m_tabCtrl.GetCurSel();
	switch (nSel)
	{
...
void CAccountDlg::BanAccount()
{
	...
	{
		CString strErr = theApp.m_iniFile.ReadStringValue(INI_ERROR_PROMPT_SECTION, "AccountNickNameInput");
	...
	else if (nNickNameLen>0 && nAccountLen>0)
	{
		...
		for (i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
		{
			if (g_sGameOperInfo[i].uGameID == nGameID)
			{
				bBanNick = g_sGameOperInfo[i].bOperByNick;
				break;
			}
		}
		ASSERT(i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO));
	}
	...
	if (BST_CHECKED != m_chkBtnUnbanTimeOption.GetCheck())////是否永久封停
	{
		...
		{
			CString strErr = theApp.m_iniFile.ReadStringValue(INI_ERROR_PROMPT_SECTION, "TimeErr");
			AfxMessageBox(strErr);
			return;
		}
	...
	{
		CString strErr = theApp.m_iniFile.ReadStringValue(INI_ERROR_PROMPT_SECTION, "ReasonInput");
		AfxMessageBox(strErr);
		return;
	}
...
//		}
	...
	{
		CString strErr = theApp.m_iniFile.ReadStringValue(INI_ERROR_PROMPT_SECTION, "ConstructTFLV");
		AfxMessageBox(strErr);
	}
}

// 解封帐号.
void CAccountDlg::UnBanAccount()
{
	...
	else if (nNickNameLen>0 && nAccountLen>0)
	{
		...
		for (i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
		{
			if (g_sGameOperInfo[i].uGameID == nGameID)
			{
				bUnBanNick = g_sGameOperInfo[i].bOperByNick;
				break;
			}
		}
		ASSERT(i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO));
...
void CAccountDlg::SetBanTime()
{
	...
	else if (nNickNameLen>0 && nAccountLen>0)
	{
		...
		for (i=0; i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO); i++)
		{
			if (g_sGameOperInfo[i].uGameID == nGameID)
			{
				bOperByNick = g_sGameOperInfo[i].bOperByNick;
				break;
			}
		}
		ASSERT(i<sizeof(g_sGameOperInfo)/sizeof(GAME_OPER_INFO));
...
void CAccountDlg::ClearGameNameCombBoxAssociateData()
{
	// Clear.
	std::stack<LPGAME_DB_SERVER> stackData;
	...
	for (int i=0; i<nCount; i++)
	{
		...
		if (-1 == (int)lpData)
			continue;
		LPGAME_INFO lpgi = (LPGAME_INFO)lpData;
		ASSERT(NULL != lpgi);
	...
	{
		LPGAME_DB_SERVER lpgds = (LPGAME_DB_SERVER)stackData.top();
		ASSERT(NULL != lpgds);
		delete lpgds;
		stackData.pop();
	}
...
void CAccountDlg::ClearGameDBServerCombBoxAssociateData()
{
...
//maple add
void CAccountDlg::OnBnClickBanTimeOption()
{
