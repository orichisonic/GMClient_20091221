...
IMPLEMENT_DYNAMIC(WAUserInfor, CDialog)
WAUserInfor::WAUserInfor(CWnd* pParent /*=NULL*/)
: CDialog(WAUserInfor::IDD, pParent)
{
}

WAUserInfor::~WAUserInfor()
{
}

void WAUserInfor::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(WAUserInfor, CDialog)
	ON_WM_CTLCOLOR()
	ON_WM_SIZE()
...
HBRUSH WAUserInfor::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
VOID WAUserInfor::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","UIC_UI_tpgFriendInfor"));
	InitQuestType();


}
// 查询玩家资料信息
void WAUserInfor::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 接收socket返回信息
LRESULT WAUserInfor::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::SERVERINFO_IP_QUERY_RESP://查询服务器列表信息的回应
		{
			...
			ServerList.Lookup(cmbServer.GetCurSel()+1,ServerIP);
		}
		break;
	case CEnumCore::Message_Tag_ID::WA_Account_Query_Resp://查询玩家资料信息的回应
	case CEnumCore::Message_Tag_ID::WA_FriendInfo_Query_Resp://查询玩家好友信息的回应
		{
...
// 显示角色资料信息
void WAUserInfor::ShowUserInfo(void)
{
...
// 显示好友列表信息
void WAUserInfor::ShowFriendInfo(void)
{
...
// 单击用户列表保存当前行号
void WAUserInfor::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void WAUserInfor::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationWA operWA;
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void WAUserInfor::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationWA operWA;
	if(tbcResult.GetCurSel() == 0)
	{
...
//把是否已经删除改变颜色
void WAUserInfor::Changecolumncolor()
{
	CString isdel;
 for (int i=0;i<listUser.GetItemCount();i++)
 {
	 isdel=theApp.operWA.GetValueByColName(&listUser, nItem, theApp.operWA.ReadValue("MWA","WA_IsDelete"));
	 if(isdel=="1")
	 {
		//listUser.GetItem()
	 }
...
void WAUserInfor::FriendList()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_FriendInfo_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operWA.ReadIntValue("MWA","Send");//当前的状态为发送查询请求
		}
...
//初始化任务类型
void WAUserInfor::InitQuestType(void)
{
...
// 自动改变大小
void WAUserInfor::OnSize(UINT nType, int cx, int cy)
{
...
// 翻页进行查询
void WAUserInfor::OnCbnSelchangeCmbpage()
{
	try
	{	
		//CString temp;
		//temp.Format("%s",pageInfo);
		//MessageBox(temp);
...
//处理快捷键响应函数
BOOL WAUserInfor::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operXD;
