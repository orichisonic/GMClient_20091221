...
IMPLEMENT_DYNAMIC(CWAMailLog, CDialog)
CWAMailLog::CWAMailLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWAMailLog::IDD, pParent)
{
}

CWAMailLog::~CWAMailLog()
{
}

void CWAMailLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWAMailLog, CDialog)
...
// 初始化显示字符
void CWAMailLog::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","ML_UI_MailLog"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","ML_UI_MailLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","ML_UI_MailSend"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","ML_UI_MailRecv"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","ML_UI_MailDel"));
}


// 自动改变大小
void CWAMailLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWAMailLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWAMailLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWAMailLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWAMailLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWAMailLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWAMailLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWAMailLog::OnBnClickedBtnlog()
{
	pageInfo = false;
	cmbPage.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
	...
	CharMailLog(m_LogType);
}

//角色邮件日志
void CWAMailLog::CharMailLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","ML_UI_MailSend"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MailSend);//邮件发送记录
			else if(logType==theApp.operWA.ReadValue("MWA","ML_UI_MailRecv"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MailRecv);//邮件接收记录
			else if(logType==theApp.operWA.ReadValue("MWA","ML_UI_MailDel"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MailDelete);//邮件删除记录
	...
	{
		MessageBox(theApp.operWA.ReadValue("MWA","UIC_Error_MailLog"),theApp.operWA.ReadValue("MWA","Error"),0);
	}	
}

// 翻页进行查询
void CWAMailLog::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWAMailLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWAMailLog::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWAMailLog::PreTranslateMessage(MSG* pMsg)
{
