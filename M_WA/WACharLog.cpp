...
IMPLEMENT_DYNAMIC(CWACharLog, CDialog)
CWACharLog::CWACharLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWACharLog::IDD, pParent)
{
}

CWACharLog::~CWACharLog()
{
}

void CWACharLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWACharLog, CDialog)
...
// 初始化显示字符
void CWACharLog::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","CL_UI_CharLog"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","CL_UI_CharLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_Upgrade"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_Create"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_Del"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_AddGuild"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_LeaveGuild"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_CreateGuild"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_DisbandGuild"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_ChangeJob"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_SkillLog"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_LiveSkill"));
}


// 自动改变大小
void CWACharLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWACharLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWACharLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWACharLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWACharLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWACharLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWACharLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWACharLog::OnBnClickedBtnlog()
{
	pageInfo = false;
	cmbPage.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
...
//角色日志
void CWACharLog::CharLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","CL_UI_Upgrade"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Character_Upgrade);//角色升级记录
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_Create"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Character_Create);//角色创建记录
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_Del"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Character_Delete);//角色删除记录
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_AddGuild"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Join_Guild);//加入帮会
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_LeaveGuild"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Quit_Guild);//离开帮会
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_CreateGuild"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Create_Guild);//创建帮会
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_DisbandGuild"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Dissolve_Guild);//解散帮会
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_ChangeJob"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Transfer_Info);//转职信息查询
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_SkillLog"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Skills_Log);//技能日志记录
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_LiveSkill"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_LifeSkills_Log);//生活技能日志记录
...
// 翻页进行查询
void CWACharLog::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWACharLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWACharLog::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWACharLog::PreTranslateMessage(MSG* pMsg)
{
