...
IMPLEMENT_DYNAMIC(CWAUserLog, CDialog)
CWAUserLog::CWAUserLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWAUserLog::IDD, pParent)
{
}

CWAUserLog::~CWAUserLog()
{
}

void CWAUserLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWAUserLog, CDialog)
...
// 初始化显示字符
void CWAUserLog::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","UL_UI_UserLog"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","UL_UI_UserLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","UL_UI_Instance"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","UL_UI_LoginLogout"));
}


// 自动改变大小
void CWAUserLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWAUserLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWAUserLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWAUserLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWAUserLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWAUserLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWAUserLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWAUserLog::OnBnClickedBtnlog()
{
	pageInfo = false;
	cmbPage.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
	...
	CharUserLog(m_LogType);
}


//角色日志
void CWAUserLog::CharUserLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","UL_UI_Instance"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_Instance);//查询用户的副本日志
			else if(logType==theApp.operWA.ReadValue("MWA","UL_UI_LoginLogout"))			
	...
	{
		MessageBox(theApp.operWA.ReadValue("MWA","UIC_Error_UserLog"),theApp.operWA.ReadValue("MWA","Error"),0);
	}	
}

// 翻页进行查询
void CWAUserLog::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWAUserLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWAUserLog::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWAUserLog::PreTranslateMessage(MSG* pMsg)
{
