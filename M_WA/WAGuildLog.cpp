...
IMPLEMENT_DYNAMIC(CWAGuildLog, CDialog)
CWAGuildLog::CWAGuildLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWAGuildLog::IDD, pParent)
{
}

CWAGuildLog::~CWAGuildLog()
{
}

void CWAGuildLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWAGuildLog, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
...
void CWAGuildLog::InitUI()
{
	...
	SetDlgItemText(IDC_LBLNAME,theApp.operWA.ReadValue("MWA","GI_UI_GuildName"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","EL_UI_EmployeeLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","EL_UI_Create"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","EL_UI_Disband"));
}


// 自动改变大小
void CWAGuildLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWAGuildLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CWAGuildLog::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CWAGuildLog::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CWAGuildLog::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击基本资料信息进行查询
void CWAGuildLog::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWAGuildLog::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWAGuildLog::OnBnClickedBtnlog()
{
	pageResult = false;
	cmbResult.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
...
//查询日志信息
void CWAGuildLog::ResultLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","EL_UI_Create"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Create_MercenaryGroup);//创建记录
			else if(logType==theApp.operWA.ReadValue("MWA","EL_UI_Disband"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Dissolve_MercenaryGroup);//解散记录
...
// 翻页进行查询
void CWAGuildLog::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CWAGuildLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWAGuildLog::OnBnClickedCancel()
{
...
//全选、复制
BOOL CWAGuildLog::PreTranslateMessage(MSG* pMsg)
{
