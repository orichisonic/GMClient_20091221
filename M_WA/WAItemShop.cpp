...
IMPLEMENT_DYNAMIC(CWAItemShop, CDialog)
CWAItemShop::CWAItemShop(CWnd* pParent /*=NULL*/)
	: CDialog(CWAItemShop::IDD, pParent)
{
}

CWAItemShop::~CWAItemShop()
{
}

void CWAItemShop::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECKSENDER, checkSender);
	DDX_Control(pDX, IDC_CHECKRECEIVER, checkReceiver);
}


BEGIN_MESSAGE_MAP(CWAItemShop, CDialog)
	...
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_CHECKSENDER, OnBnClickedChecksender)
	ON_BN_CLICKED(IDC_CHECKRECEIVER, OnBnClickedCheckreceiver)
END_MESSAGE_MAP()
...
BOOL CWAItemShop::OnInitDialog()
{
	...
	UserName = "";
	UserNick = "";//初始化一些私有变量
	checkSender.SetCheck(1);
	checkReceiver.SetCheck(0);
	//按照赠送人进行查询
	userType=1;
...
// 初始化显示字符
void CWAItemShop::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","SI_UI_ItemShop"));
	...
	SetDlgItemText(IDC_CHECKSENDER,theApp.operWA.ReadValue("MWA","SI_UI_RadioSender"));
	SetDlgItemText(IDC_CHECKRECEIVER,theApp.operWA.ReadValue("MWA","SI_UI_RadioReceiver"));

	tbcResult.InsertItem(0,theApp.operWA.ReadValue("MWA","UIC_UI_tpgUserInfo"));
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","SI_UI_ItemShop"));
}


// 自动改变大小
void CWAItemShop::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWAItemShop::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWAItemShop::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWAItemShop::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWAItemShop::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWAItemShop::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWAItemShop::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//按照赠送人信息查询
void CWAItemShop::OnBnClickedChecksender()
{
	if(checkReceiver.GetCheck()==1)
	{
		checkReceiver.SetCheck(0);
		checkSender.SetCheck(1);
		userType=1;
...
//按照受赠人信息查询
void CWAItemShop::OnBnClickedCheckreceiver()
{
	if(checkSender.GetCheck()==1)
	{
		checkReceiver.SetCheck(1);
		checkSender.SetCheck(0);
		userType=2;
		if(tbcResult.GetCurSel()!=0)
			ItemShopInfo();
	}
...
void CWAItemShop::ItemShopInfo()
{
	...
	{		
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			socketdata.AddTFLV(CEnumCore::TagName::WA_USERTYPE,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//1.赠送人，2.受赠人

			int midlength = socketdata.EnBody();
			theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_ItemShop);//商城购买记录查询
	...
	{
		MessageBox(theApp.operWA.ReadValue("MWA","UIC_Error_ItemShop"),theApp.operWA.ReadValue("MWA","Error"),0);
	}	
}

// 翻页进行查询
void CWAItemShop::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWAItemShop::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWAItemShop::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWAItemShop::PreTranslateMessage(MSG* pMsg)
{
