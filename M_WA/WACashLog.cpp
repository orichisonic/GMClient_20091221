...
IMPLEMENT_DYNAMIC(CWACashLog, CDialog)
CWACashLog::CWACashLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWACashLog::IDD, pParent)
{
}

CWACashLog::~CWACashLog()
{
}

void CWACashLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWACashLog, CDialog)
...
// 初始化显示字符
void CWACashLog::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","CL_UI_CashLog"));
	...
	SetDlgItemText(IDC_LBLLOGTYPE,theApp.operWA.ReadValue("MWA","CL_UI_LogType"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","CL_UI_CashLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_MonsterDrop"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_QuestGet"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_Repair"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_TRADEGET"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","CL_UI_TRADELOSE"));
}


// 自动改变大小
void CWACashLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWACashLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWACashLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWACashLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWACashLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWACashLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWACashLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWACashLog::OnBnClickedBtnlog()
{
	pageInfo = false;
	cmbPage.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
	...
	CharCashLog(m_LogType);
}

//角色游戏币日志
void CWACashLog::CharCashLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","CL_UI_MonsterDrop"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MonsterDropMoney);//怪物掉落
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_QuestGet"))			
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_GetMoneyQuest);//任务获得
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_Repair"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_LoseMoneyRepair);//修装
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_TRADEGET"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_GetMoneyTrade);//交易获得
			else if(logType==theApp.operWA.ReadValue("MWA","CL_UI_TRADELOSE"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_LoseMoneyTrade);//交易失去
	...
	{
		MessageBox(theApp.operWA.ReadValue("MWA","UIC_Error_CashLog"),theApp.operWA.ReadValue("MWA","Error"),0);
	}	
}

// 翻页进行查询
void CWACashLog::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWACashLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWACashLog::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWACashLog::PreTranslateMessage(MSG* pMsg)
{
