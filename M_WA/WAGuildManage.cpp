	...
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() != 0)
		{
			...
			{			
				...
				GuildName = theApp.operWA.GetValueByColName(&listBasic, nItem, theApp.operWA.ReadValue("MWA","WA_GuildName"));// 保存选中的公会信息
...
{		
	...
	else if(tbcResult.GetCurSel() == 2)//选中的是结果资料信息选项卡
	{
		pageResult = false;
		cmbResult.ResetContent();
		while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
...
void CWAGuildManage::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(listResult.GetItemCount() != 0)
	{
		...
		if(mItem >= 0)
		{			
			UserName = theApp.operWA.GetValueByColName(&listResult, mItem, theApp.operWA.ReadValue("MWA","WA_Account"));// 保存选中的玩家帐号
			UserID = theApp.operWA.GetValueByColName(&listResult, mItem, theApp.operWA.ReadValue("MWA","WA_NickID"));// 保存选中的玩家ID
			UserNick = theApp.operWA.GetValueByColName(&listResult, mItem, theApp.operWA.ReadValue("MWA","WA_UserNick"));// 保存选中的玩家昵称
...
void CWAGuildManage::OnBnClickedBtndel()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(tbcResult.GetCurSel() == 0)
			theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_DissolveGuild_Operate);//解散公会
		else if(tbcResult.GetCurSel() ==1)
			theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_KickGuildMember_Operate);//踢出公会成员
...
void CWAGuildManage::OnBnClickedBtntransfer()
{
	...
	{		
		...
		{		
			GetDlgItemText(IDC_EDITEDITNAME,UserNick);//获取玩家帐号
			if(UserNick == "")//要转让佣兵团团长的权限的玩家帐号不能为空
			{
				MessageBox(theApp.operWA.ReadValue("MWA","GM_HINT_PARAMNULL"),theApp.operWA.ReadValue("MWA","Warn"),MB_ICONWARNING+MB_OK);
				return;
			}
			...
			int midlength = socketdata.EnBody();
			theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_AttornPopedom_Operate);//转让佣兵团团长的权限
...
void CWAGuildManage::OnBnClickedBtnedit()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			GetDlgItemText(IDC_EDITNEWVALUE,NewGuildName);//获取公会名称
			if(NewGuildName == "")//修改后的公会名称不能为空
			{
				MessageBox(theApp.operWA.ReadValue("MWA","GM_HINT_PARAMNULL"),theApp.operWA.ReadValue("MWA","Warn"),MB_ICONWARNING+MB_OK);
				return;
			}
			...
			int midlength = socketdata.EnBody();
			theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_ModifyGuildName_Operate);//修改公会名称
