...
IMPLEMENT_DYNAMIC(CWAItemLog, CDialog)
CWAItemLog::CWAItemLog(CWnd* pParent /*=NULL*/)
	: CDialog(CWAItemLog::IDD, pParent)
{
}

CWAItemLog::~CWAItemLog()
{
}

void CWAItemLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWAItemLog, CDialog)
...
// 初始化显示字符
void CWAItemLog::InitUI()
{
	SetWindowText(theApp.operWA.ReadValue("MWA","IL_UI_ItemLog"));
	...
	SetDlgItemText(IDC_LBLLOGTYPE,theApp.operWA.ReadValue("MWA","IL_UI_LogType"));
	...
	tbcResult.InsertItem(1,theApp.operWA.ReadValue("MWA","IL_UI_ItemLog"));

	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_MonsterDrop"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_BuyFromNPC"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_SellToNPC"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_SELFDEL"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_TRADEGET"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_TRADELOSE"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_BINDEQUIP"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_UNBINDEQUIP"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_BOOTHITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_DELITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_USEITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_CRAFTITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_PUNCHITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_CHANGESLOT"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_UPGRADESTAR"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_GEMITEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_PHARMA"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_CRAFTGEM"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_MATERIAL"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_CRAFTRECIPE"));
	cmbLogType.AddString(theApp.operWA.ReadValue("MWA","IL_UI_MazeIntegral"));
}


// 自动改变大小
void CWAItemLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CWAItemLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CWAItemLog::OnBnClickedUisearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询玩家资料信息
void CWAItemLog::OnCbnSelchangeCmbuserpage()
{	
	try
	{	
...
// 单击用户列表保存当前行号
void CWAItemLog::OnNMClickListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击玩家资料信息进行查询
void CWAItemLog::OnNMDblclkListcharacter(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CWAItemLog::OnTcnSelchangeTabuiresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CWAItemLog::OnBnClickedBtnlog()
{
	pageInfo = false;
	cmbPage.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
	...
	CharItemLog(m_LogType);
}

//角色物品日志
void CWAItemLog::CharItemLog(CString logType)
{
	try
	{		
		CIni mIni;
		...
		if((listUser.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(logType==theApp.operWA.ReadValue("MWA","IL_UI_MonsterDrop"))
				theApp.operWA.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MonsterDropItem);//怪物掉落物品
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_BuyFromNPC"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_BuyItemNPC);//从NPC处购买
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_SellToNPC"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_SellItemTONPC);//卖给NPC
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_SELFDEL"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_DropItem);//玩家自行删除
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_TRADEGET"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_GetItemTrade);//交易获得
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_TRADELOSE"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_LoseItemTrade);//交易失去
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_BINDEQUIP"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_BindItem);//装备绑定
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_UNBINDEQUIP"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_UnBindItem);//装备解绑
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_BOOTHITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_BoothItem);//玩家摆摊
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_DELITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_DeleteItem);//道具删除
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_USEITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_UseItem);//道具使用
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_CRAFTITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_CraftItem);//装备制作
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_PUNCHITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_PunchItem);//装备打孔
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_CHANGESLOT"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_ChangeSlotItem);//装备改孔
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_UPGRADESTAR"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_UpgradeStar);//装备升星
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_GEMITEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_Gem);//宝石
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_PHARMA"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_Pharma);//制药记录
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_CRAFTGEM"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_CraftGem);//宝石合成
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_MATERIAL"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_CraftMatirail);//材料合成
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_CRAFTRECIPE"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_CraftRecipe);//图纸合成
			else if(logType==theApp.operWA.ReadValue("MWA","IL_UI_MazeIntegral"))
				theApp.operWA.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::WA_Query_MazeIntegral);//迷宫积分兑换奖励
	...
	{
		MessageBox(theApp.operWA.ReadValue("MWA","UIC_Error_ItemLog"),theApp.operWA.ReadValue("MWA","Error"),0);
	}	
}

// 翻页进行查询
void CWAItemLog::OnCbnSelchangeCmbuipage()
{	
	try
	{
		if(pageInfo)
...
// 接收返回信息
LRESULT CWAItemLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CWAItemLog::OnBnClickedUicancel()
{
...
//全选、复制
BOOL CWAItemLog::PreTranslateMessage(MSG* pMsg)
{
