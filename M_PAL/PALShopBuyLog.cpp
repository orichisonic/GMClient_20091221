...
IMPLEMENT_DYNAMIC(CPALShopBuyLog, CDialog)
CPALShopBuyLog::CPALShopBuyLog(CWnd* pParent /*=NULL*/)
	: CDialog(CPALShopBuyLog::IDD, pParent)
{
}

CPALShopBuyLog::~CPALShopBuyLog()
{
}

void CPALShopBuyLog::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_GRPSEARCH, GrpSearch);
	DDX_Control(pDX, IDC_CMBSERVER, cmbServer);
	DDX_Control(pDX, IDC_CHECKBUY, checkBuy);
	DDX_Control(pDX, IDC_CHECKSEND, checkSend);
...
BEGIN_MESSAGE_MAP(CPALShopBuyLog, CDialog)
	ON_BN_CLICKED(IDC_CHECKBUY, OnBnClickedCheckbuy)
	ON_BN_CLICKED(IDC_CHECKSEND, OnBnClickedChecksend)
...
// 初始化对话框
BOOL CPALShopBuyLog::OnInitDialog()
{
	...
	GetDlgItem(IDC_CHECKBUY)->ShowWindow(SW_HIDE);
...
// 初始化显示字符
void CPALShopBuyLog::InitUI()
{
	//OperationPAL operPAL;

	SetWindowText(theApp.operPAL.ReadValue("MPAL","SL_UI_ShopBuyLog"));
	...
	SetDlgItemText(IDC_CHECKBUY,theApp.operPAL.ReadValue("MPAL","SL_UI_UserAsBuyer"));
	SetDlgItemText(IDC_CHECKSEND,theApp.operPAL.ReadValue("MPAL","SL_UI_UserAsSender"));
	...
	tbcResult.InsertItem(1,theApp.operPAL.ReadValue("MPAL","SL_UI_tpgShopBuy"));
	tbcResult.InsertItem(2,theApp.operPAL.ReadValue("MPAL","SL_UI_tpgMoneyConsume"));
}

//处理快捷键响应函数
BOOL CPALShopBuyLog::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALShopBuyLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALShopBuyLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 设置作为购买方来查询
void CPALShopBuyLog::OnBnClickedCheckbuy()
{
	if(checkBuy.GetCheck() == 1)
	{
		checkSend.SetCheck(0);
	}
}

// 设置作为发送方来查询
void CPALShopBuyLog::OnBnClickedChecksend()
{
	if(checkSend.GetCheck() == 1)
	{
		checkBuy.SetCheck(0);
	}
}
...
// 查询玩家资料信息
void CPALShopBuyLog::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALShopBuyLog::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALShopBuyLog::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CPALShopBuyLog::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
//查询玩家商城购买记录
void CPALShopBuyLog::ShopBuyLog()
{
	//OperationPAL operPAL;	
	try
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PALLOG_ITEMMAIL))
		{
...
//查询玩家元宝消耗记录
void CPALShopBuyLog::MoneyConsumeLog()
{
	//OperationPAL operPAL;	
	try
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_ITEMMAIL_WASTE))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
// 翻页进行查询
void CPALShopBuyLog::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
		if(pageInfo)//如果可以翻页的话
		{
			...
			case 1:
				{
					...
					if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PALLOG_ITEMMAIL))//发送查询玩家商城购买记录的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
			...
			case 2:
				{
					midlength=socketdata.EnBody();
					if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_ITEMMAIL_WASTE))//发送查询玩家元宝消耗记录的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALShopBuyLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PALLOG_ITEMMAIL_RESP://查询玩家商城购买记录的回应
	case CEnumCore::Message_Tag_ID::PAL_ITEMMAIL_WASTE_RESP://查询玩家元宝消耗记录的回应
		{
...
//单击用户资料信息的列名进行排序
void CPALShopBuyLog::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALShopBuyLog::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALShopBuyLog::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALShopBuyLog::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALShopBuyLog::ShowOtherInfo(void)
{
