...
IMPLEMENT_DYNAMIC(CPALItemInfo, CDialog)
CPALItemInfo::CPALItemInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALItemInfo::IDD, pParent)
{
}

CPALItemInfo::~CPALItemInfo()
{
}

void CPALItemInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALItemInfo, CDialog)
...
// 初始化对话框
BOOL CPALItemInfo::OnInitDialog()
{
...
//处理快捷键响应函数
BOOL CPALItemInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALItemInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALItemInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALItemInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALItemInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALItemInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CPALItemInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
//查询玩家背包信息
void CPALItemInfo::BagInfo(int iBagType)
{
	//OperationPAL operPAL;	
	try
	{		
...
// 翻页进行查询
void CPALItemInfo::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
			...
			case 2:
				{
					...
					socketdata.AddTFLV(CEnumCore::TagName::PAL_BAGTYPE,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//页数
					midlength=socketdata.EnBody();
					if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERBAG))//发送查询玩家背包信息的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALItemInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//单击用户资料信息的列名进行排序
void CPALItemInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALItemInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALItemInfo::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALItemInfo::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALItemInfo::ShowOtherInfo(void)
{
