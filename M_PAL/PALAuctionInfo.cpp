			...
			case 1:
				{
					...
					if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_AUCTION_QUERY))//发送查询玩家拍卖信息的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			            dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
LRESULT CPALAuctionInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_AUCTION_QUERY_RESP://查询玩家拍卖信息回应
		{
