...
IMPLEMENT_DYNAMIC(CPALUserInfo, CDialog)
CPALUserInfo::CPALUserInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALUserInfo::IDD, pParent)
{
}

CPALUserInfo::~CPALUserInfo()
{
}

void CPALUserInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALUserInfo, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_CMBQUESTTYPE, OnCbnSelchangeCmbquesttype)
...
// 初始化对话框
BOOL CPALUserInfo::OnInitDialog()
{
...
void CPALUserInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgUserFriendList"));
	tbcResult.InsertItem(2,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgUserQuest"));
	tbcResult.InsertItem(3,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgPetBag"));
	//tbcResult.InsertItem(4,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgAdvanceInfo"));//取得角色进阶资讯
	//tbcResult.InsertItem(5,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgSkill"));//取得角色技能资讯
	//tbcResult.InsertItem(6,theApp.operPAL.ReadValue("MPAL","UIC_UI_tpgBlackList"));//取得角色黑名单列表

}

//处理快捷键响应函数
BOOL CPALUserInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALUserInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALUserInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALUserInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALUserInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALUserInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationPAL operPAL;
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CPALUserInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationPAL operPAL;
	if(tbcResult.GetCurSel() == 0)
	{
	...
	else
	{
		...
		if((listUser.GetItemCount() > 0) && (nItem >= 0))
		{
			...
			{
				AdvanceInfo();//取得角色进阶资讯
			}
			else if(tbcResult.GetCurSel()==5)
			{
				CharacterSkill();//取得角色技能资讯
			}
...
void CPALUserInfo::FriendList()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERFRIENDLIST))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
void CPALUserInfo::AdvanceInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERADVANCEINFO))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
//取得角色技能资讯
void CPALUserInfo::CharacterSkill()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERSKILL))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
//取得角色伏魔资讯
void CPALUserInfo::CharacterMob()
{
	try
	{		
		CIni mIni;
...
void CPALUserInfo::BlackList()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERBLACKLIST))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
//切换任务状态查询玩家任务信息
void CPALUserInfo::OnCbnSelchangeCmbquesttype()
{
	try
	{		
		CIni mIni;
...
// 翻页进行查询
void CPALUserInfo::OnCbnSelchangeCmbpage()
{
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERFRIENDLIST))//发送查询玩家好友列表的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
			...
			case 3:
				{
					...
					midlength=socketdata.EnBody();
					if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERBAG))//发送查询玩家御灵信息的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALUserInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//单击用户资料信息的列名进行排序
void CPALUserInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALUserInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALUserInfo::OnBnClickedCancel()
{
...
//初始化任务类型
void CPALUserInfo::InitQuestType(void)
{
	//OperationPAL operPAL;
...
// 显示角色资料信息
void CPALUserInfo::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALUserInfo::ShowOtherInfo(void)
{
