...
// CPALCDKeyItemGet 对话框

IMPLEMENT_DYNAMIC(CPALCDKeyItemGet, CDialog)
CPALCDKeyItemGet::CPALCDKeyItemGet(CWnd* pParent /*=NULL*/)
	: CDialog(CPALCDKeyItemGet::IDD, pParent)
{
}

CPALCDKeyItemGet::~CPALCDKeyItemGet()
{
}

void CPALCDKeyItemGet::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALCDKeyItemGet, CDialog)
...
// 初始化对话框
BOOL CPALCDKeyItemGet::OnInitDialog()
{
...
// 初始化显示字符
void CPALCDKeyItemGet::InitUI()
{
	//////OperationPAL operPAL;

	SetWindowText(theApp.operPAL.ReadValue("MPAL","IG_UI_CDKeyItemGet"));
	...
	tbcResult.InsertItem(1,theApp.operPAL.ReadValue("MPAL","IG_UI_GameItemGet"));
}

//处理快捷键响应函数
BOOL CPALCDKeyItemGet::PreTranslateMessage(MSG* pMsg)
{
	//////OperationPAL operPAL;
...
// 自动改变大小
void CPALCDKeyItemGet::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALCDKeyItemGet::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALCDKeyItemGet::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALCDKeyItemGet::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALCDKeyItemGet::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CPALCDKeyItemGet::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
//查询玩家游戏领取情况
void CPALCDKeyItemGet::GameItemGet()
{
	//////OperationPAL operPAL;	
	try
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CDKEY_QUERY))
		{
...
// 翻页进行查询
void CPALCDKeyItemGet::OnCbnSelchangeCmbpage()
{
	//////OperationPAL operPAL;
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CDKEY_QUERY))//发送查询玩家商城购买记录的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
		             	dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALCDKeyItemGet::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_CDKEY_QUERY_RESP://查询玩家游戏领取情况的回应
		{
...
//单击用户资料信息的列名进行排序
void CPALCDKeyItemGet::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALCDKeyItemGet::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALCDKeyItemGet::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALCDKeyItemGet::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALCDKeyItemGet::ShowOtherInfo(void)
{
