...
// CPALPuzzleMapInfo 对话框

IMPLEMENT_DYNAMIC(CPALPuzzleMapInfo, CDialog)
CPALPuzzleMapInfo::CPALPuzzleMapInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALPuzzleMapInfo::IDD, pParent)
{
}

CPALPuzzleMapInfo::~CPALPuzzleMapInfo()
{
}

void CPALPuzzleMapInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALPuzzleMapInfo, CDialog)
...
// 初始化对话框
BOOL CPALPuzzleMapInfo::OnInitDialog()
{
...
//处理快捷键响应函数
BOOL CPALPuzzleMapInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALPuzzleMapInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALPuzzleMapInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALPuzzleMapInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALPuzzleMapInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALPuzzleMapInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CPALPuzzleMapInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
//查询玩家图鉴
void CPALPuzzleMapInfo::PuzzleMap()
{
	//OperationPAL operPAL;	
	try
	{		
...
// 翻页进行查询
void CPALPuzzleMapInfo::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
...
// 接收返回信息
LRESULT CPALPuzzleMapInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//单击用户资料信息的列名进行排序
void CPALPuzzleMapInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALPuzzleMapInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALPuzzleMapInfo::OnBnClickedCancel()
{
...
// 关闭对话框
void CPALPuzzleMapInfo::OnClose()
{
...
// 显示角色资料信息
void CPALPuzzleMapInfo::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALPuzzleMapInfo::ShowOtherInfo(void)
{
