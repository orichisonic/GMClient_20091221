...
IMPLEMENT_DYNAMIC(CPALMailBoxInfo, CDialog)
CPALMailBoxInfo::CPALMailBoxInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALMailBoxInfo::IDD, pParent)
{
}

CPALMailBoxInfo::~CPALMailBoxInfo()
{
}

void CPALMailBoxInfo::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LISTMAILDETAIL, listMailDetail);
}


BEGIN_MESSAGE_MAP(CPALMailBoxInfo, CDialog)
...
// 初始化对话框
BOOL CPALMailBoxInfo::OnInitDialog()
{
...
void CPALMailBoxInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operPAL.ReadValue("MPAL","BI_UI_tpgMailBoxInfo"));
	tbcResult.InsertItem(2,theApp.operPAL.ReadValue("MPAL","BI_UI_tpgMailBoxDetail"));
}

//处理快捷键响应函数
BOOL CPALMailBoxInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALMailBoxInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALMailBoxInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALMailBoxInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALMailBoxInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 单击邮箱列表保存当前行号
void CPALMailBoxInfo::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CPALMailBoxInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 双击玩家邮箱邮件进行查询
void CPALMailBoxInfo::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 1)
	{
		...
		if(nItem >= 0)
		{
			...
			pageInfo = false;
			cmbPage.ResetContent();
			while(listMailDetail.DeleteColumn(0)){}// 清除结果列表中的所有内容

			//OperationPAL operPAL;
			MailID = atoi(theApp.operPAL.GetValueByColName(&listResult, nItem, theApp.operPAL.ReadValue("MPAL","PAL_MAILID")));//保存选中的MAILID
			MailBoxDetail();// 查询玩家邮箱信息
		}
	}
	else
	{
...
// 切换选项卡进行查询
void CPALMailBoxInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
	...
	else if(tbcResult.GetCurSel() == 1)
	{
			...
			{
				//OperationPAL operPAL;
				UserID = atoi(theApp.operPAL.GetValueByColName(&listUser, nItem, theApp.operPAL.ReadValue("MPAL","PAL_ROLEID")));//保存选中的帐号ID
				UserAccount = theApp.operPAL.GetValueByColName(&listUser, nItem, theApp.operPAL.ReadValue("MPAL","PAL_ACCOUNT"));// 保存选中的玩家帐号
				UserName = theApp.operPAL.GetValueByColName(&listUser, nItem, theApp.operPAL.ReadValue("MPAL","PAL_ROLENAME"));// 保存选中的玩家角色名
	...
	else
	{
		...
		if((listResult.GetItemCount() > 0) && (nMailItem >= 0))
		{

			MailID = atoi(theApp.operPAL.GetValueByColName(&listResult, nMailItem, theApp.operPAL.ReadValue("MPAL","PAL_MAILID")));//保存选中的MAILID
...
//查询玩家邮箱信息
void CPALMailBoxInfo::MailBoxInfo()
{
	//OperationPAL operPAL;	
	try
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHECKMAILBOX_QUERY))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
//查询玩家邮箱附件信息
void CPALMailBoxInfo::MailBoxDetail()
{
	//OperationPAL operPAL;	
	try
	{		
		...
		_itoa(MailID,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::PAL_MAILID,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//MAILID
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHECKMAILBOX_DETAIL))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
// 翻页进行查询
void CPALMailBoxInfo::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHECKMAILBOX_QUERY))//发送查询玩家邮箱信息的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
		            	dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALMailBoxInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_CHECKMAILBOX_QUERY_RESP://查询玩家邮箱信息回应
		{
...
//单击用户资料信息的列名进行排序
void CPALMailBoxInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALMailBoxInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALMailBoxInfo::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALMailBoxInfo::ShowUserInfo(void)
{
...
// 显示邮箱信息
void CPALMailBoxInfo::ShowMailInfo(void)
{
	tbcResult.SetCurSel(1);
...
// 显示邮件附件界面
void CPALMailBoxInfo::ShowMailDetail(void)
{
	tbcResult.SetCurSel(2);
	GetDlgItem(IDC_LISTUSERINFO)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_GRPPAGE)->ShowWindow(SW_SHOW);
