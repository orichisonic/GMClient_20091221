...
IMPLEMENT_DYNAMIC(CPALHisLogInfo, CDialog)
CPALHisLogInfo::CPALHisLogInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALHisLogInfo::IDD, pParent)
{
}

CPALHisLogInfo::~CPALHisLogInfo()
{
}



void CPALHisLogInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALHisLogInfo, CDialog)
...
// CPALHisLogInfo 消息处理程序

// 初始化对话框
BOOL CPALHisLogInfo::OnInitDialog()
{
...
// 初始化显示字符
void CPALHisLogInfo::InitUI()
{
	//OperationPAL operPAL;

	SetWindowText(theApp.operPAL.ReadValue("MPAL","LI_UI_HisLogInfo"));
...
//处理快捷键响应函数
BOOL CPALHisLogInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALHisLogInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALHisLogInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CPALHisLogInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击玩家资料信息保存当前行号
void CPALHisLogInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行切换
void CPALHisLogInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listUser.GetItemCount() > 0)//如果玩家资料信息不为空
...
// 切换选项卡进行查询
void CPALHisLogInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//获取所有大类信息
void CPALHisLogInfo::GetAllBigType()
{	
	try
	{	
		CIni mIni;	
...
// 选择大类查询小类
void CPALHisLogInfo::OnCbnSelchangeCmbbigtype()
{
	try
	{	
		CIni mIni;	
...
// 查询日志信息
void CPALHisLogInfo::OnBnClickedBtnsearchlog()
{
	try
	{	
		CIni mIni;	
		...
		int midlength = socketdata.EnBody();
		if(theApp.operPAL.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_HISTORY_LOGINFO_QUERY))//查询日志信息
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
		}
...
// 翻页进行查询
void CPALHisLogInfo::OnCbnSelchangeCmbpage()
{
	try
	{	
		if(pageInfo)
...
//单击用户信息进行排序
void CPALHisLogInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击日志列表信息进行排序
void CPALHisLogInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 接收返回信息
LRESULT CPALHisLogInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_HISTORY_LOGINFO_QUERY_RESP://查询玩家日志信息的回应
		{
...
//显示用户信息界面
void CPALHisLogInfo::ShowUserInfo()
{
...
//显示日志信息界面
void CPALHisLogInfo::ShowLogInfo()
{
...
//关闭对话框
void CPALHisLogInfo::OnBnClickedCancel()
{
