// PALBankInfo.cpp : 实现文件
//

#include "stdafx.h"
#include "M_PAL.h"
#include "PALBankInfo.h"
#include ".\palbankinfo.h"


// CPALBankInfo 对话框

IMPLEMENT_DYNAMIC(CPALBankInfo, CDialog)
CPALBankInfo::CPALBankInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALBankInfo::IDD, pParent)
{
}

CPALBankInfo::~CPALBankInfo()
{
}

void CPALBankInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALBankInfo, CDialog)
...
// 初始化对话框
BOOL CPALBankInfo::OnInitDialog()
{
...
// 初始化显示字符
void CPALBankInfo::InitUI()
{
	//OperationPAL operPAL;

	SetWindowText(theApp.operPAL.ReadValue("MPAL","BI_UI_BankInfo"));
	...
	tbcResult.InsertItem(1,theApp.operPAL.ReadValue("MPAL","BI_UI_tpgBankInfo"));
}

//处理快捷键响应函数
BOOL CPALBankInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALBankInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALBankInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALBankInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALBankInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALBankInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() != 0)
...
// 切换选项卡进行查询
void CPALBankInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
// 翻页进行查询
void CPALBankInfo::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operPAL.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::PAL_CHARACTERBAG))//发送查询玩家仓库信息的请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operPAL.ReadIntValue("MPAL","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CPALBankInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERBAG_RESP://查询玩家仓库信息回应
		{
...
//单击用户资料信息的列名进行排序
void CPALBankInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationPAL operPAL;
...
//单击结果信息的列名进行排序
void CPALBankInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	//OperationPAL operPAL;
...
// 关闭对话框
void CPALBankInfo::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALBankInfo::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALBankInfo::ShowOtherInfo(void)
{
