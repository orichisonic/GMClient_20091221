...
IMPLEMENT_DYNAMIC(CPALMobInfo, CDialog)
CPALMobInfo::CPALMobInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CPALMobInfo::IDD, pParent)
{
}

CPALMobInfo::~CPALMobInfo()
{
}

void CPALMobInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPALMobInfo, CDialog)
...
//处理快捷键响应函数
BOOL CPALMobInfo::PreTranslateMessage(MSG* pMsg)
{
	//OperationPAL operPAL;
...
// 自动改变大小
void CPALMobInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CPALMobInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询玩家资料信息
void CPALMobInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 单击用户列表保存当前行号
void CPALMobInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息进行查询
void CPALMobInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listUser.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CPALMobInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		ShowUserInfo();//显示用户资料信息
...
// 翻页进行查询
void CPALMobInfo::OnCbnSelchangeCmbpage()
{
	//OperationPAL operPAL;
	try
	{	
...
// 接收返回信息
LRESULT CPALMobInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERINFO_RESP://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::PAL_CHARACTERMOB_RESP://查询玩家背包的回应
		{
...
//单击用户资料信息的列名进行排序
void CPALMobInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CPALMobInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operPAL.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CPALMobInfo::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CPALMobInfo::ShowUserInfo(void)
{
...
// 显示其它信息
void CPALMobInfo::ShowOtherInfo(void)
{
