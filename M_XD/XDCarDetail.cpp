...
{		
	...
	else
	{
		...
		{
			CarID = theApp.operXD.GetValueByColName(&listResult, mItem, theApp.operXD.ReadValue("MXD","XDJG_CarID"));// 保存选中的金刚车辆ID			
			DetailInfo();//查询详细资料信息
		}
	}
	*pResult = 0;
...
void CXDCarDetail::ResultInfo()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_CarBaseInfo_Query);//金刚车辆基本信息
...
void CXDCarDetail::DetailInfo()
{
	...
	{		
		...
		if((listResult.GetItemCount() > 0) && (mItem>=0))
		{
			...
			socketdata.AddTFLV(CEnumCore::TagName::XDJG_CarID,CEnumCore::TagFormat::TLV_STRING,strlen(CarID),(unsigned char*)mIni.wcharToChar(CarID));//金刚车辆ID
			
			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_CarDetailInfo_Query);//金刚车辆详细信息
