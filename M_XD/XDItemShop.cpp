...
IMPLEMENT_DYNAMIC(CXDItemShop, CDialog)
CXDItemShop::CXDItemShop(CWnd* pParent /*=NULL*/)
	: CDialog(CXDItemShop::IDD, pParent)
{
}

CXDItemShop::~CXDItemShop()
{
}

void CXDItemShop::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDItemShop, CDialog)
	ON_WM_SIZE()
...
// 初始化显示字符
void CXDItemShop::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","SI_UI_ItemShop"));
	...
	SetDlgItemText(IDC_CHECKSENDER,theApp.operXD.ReadValue("MXD","SI_UI_RadioSender"));
	SetDlgItemText(IDC_CHECKRECEIVER,theApp.operXD.ReadValue("MXD","SI_UI_RadioReceiver"));

	tbcResult.InsertItem(0,theApp.operXD.ReadValue("MXD","UIC_UI_tpgUserInfo"));
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","SI_UI_ItemShop"));
}

// 自动改变大小
void CXDItemShop::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDItemShop::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDItemShop::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDItemShop::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDItemShop::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDItemShop::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDItemShop::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//按照赠送人信息查询
void CXDItemShop::OnBnClickedChecksender()
{
...
//按照受赠人信息查询
void CXDItemShop::OnBnClickedCheckreceiver()
{
...
void CXDItemShop::ResultInfo()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			socketdata.AddTFLV(CEnumCore::TagName::XDJG_QueryType,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//1.赠送人，2.受赠人

			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_Shoping_Log);//商城物品购买日志
...
// 翻页进行查询
void CXDItemShop::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDItemShop::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDItemShop::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDItemShop::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDItemShop::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDItemShop::ShowResultInfo(int m_cmdShow)
{
