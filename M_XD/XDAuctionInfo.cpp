...
IMPLEMENT_DYNAMIC(CXDAuctionInfo, CDialog)
CXDAuctionInfo::CXDAuctionInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CXDAuctionInfo::IDD, pParent)
{
}

CXDAuctionInfo::~CXDAuctionInfo()
{
}

void CXDAuctionInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDAuctionInfo, CDialog)
	ON_WM_SIZE()
...
// CXDAuctionInfo 消息处理程序

// 初始化对话框
BOOL CXDAuctionInfo::OnInitDialog()
{
...
// 初始化显示字符
void CXDAuctionInfo::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","AI_UI_AuctionInfo"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","AI_UI_AuctionInfo"));
}

// 自动改变大小
void CXDAuctionInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDAuctionInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDAuctionInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDAuctionInfo::OnCbnSelchangeCmbbasic()
{	
	try
	{
...
// 单击基本资料信息保存当前行号
void CXDAuctionInfo::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDAuctionInfo::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDAuctionInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询相关资料信息
void CXDAuctionInfo::ResultInfo()
{
	try
	{		
		CIni mIni;
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_AuctioneerInfo_Query);//拍卖行信息
...
// 翻页进行查询
void CXDAuctionInfo::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDAuctionInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDAuctionInfo::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDAuctionInfo::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDAuctionInfo::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDAuctionInfo::ShowResultInfo(int m_cmdShow)
{
