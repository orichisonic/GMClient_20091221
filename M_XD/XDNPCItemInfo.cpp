...
IMPLEMENT_DYNAMIC(CXDNPCItemInfo, CDialog)
CXDNPCItemInfo::CXDNPCItemInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CXDNPCItemInfo::IDD, pParent)
{
}

CXDNPCItemInfo::~CXDNPCItemInfo()
{
}

void CXDNPCItemInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDNPCItemInfo, CDialog)
	ON_WM_SIZE()
...
// CXDNPCItemInfo 消息处理程序

// 初始化对话框
BOOL CXDNPCItemInfo::OnInitDialog()
{
...
// 初始化显示字符
void CXDNPCItemInfo::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","NI_UI_NPCItemInfo"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","NI_UI_NPCItemInfo"));
}

// 自动改变大小
void CXDNPCItemInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDNPCItemInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDNPCItemInfo::OnBnClickedSearch()
{
	try
	{
...
// 翻页查询基本资料信息
void CXDNPCItemInfo::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDNPCItemInfo::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDNPCItemInfo::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDNPCItemInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询相关资料信息
void CXDNPCItemInfo::ResultInfo()
{
	try
	{		
		CIni mIni;
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			socketdata.AddTFLV(CEnumCore::TagName::XDJG_Account,CEnumCore::TagFormat::TLV_STRING,strlen(UserName),(unsigned char *)mIni.wcharToChar(UserName));//玩家帐号
			socketdata.AddTFLV(CEnumCore::TagName::XDJG_Character,CEnumCore::TagFormat::TLV_STRING,strlen(UserNick),(unsigned char *)mIni.wcharToChar(UserNick));//玩家角色名
			socketdata.AddTFLV(CEnumCore::TagName::XDJG_AccountID,CEnumCore::TagFormat::TLV_STRING,strlen(UserID),(unsigned char *)mIni.wcharToChar(UserID));//玩家账号ID
			...
			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_NPCGetItem_Query);//NPC道具领取查询
...
// 翻页进行查询
void CXDNPCItemInfo::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDNPCItemInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDNPCItemInfo::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDNPCItemInfo::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDNPCItemInfo::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDNPCItemInfo::ShowResultInfo(int m_cmdShow)
{
...
//单击基本资料信息的列名进行排序
void CXDNPCItemInfo::OnLvnColumnclickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listBasic,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CXDNPCItemInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
