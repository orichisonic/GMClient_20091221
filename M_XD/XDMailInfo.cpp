...
IMPLEMENT_DYNAMIC(CXDMailInfo, CDialog)
CXDMailInfo::CXDMailInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CXDMailInfo::IDD, pParent)
{
}

CXDMailInfo::~CXDMailInfo()
{
}

void CXDMailInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDMailInfo, CDialog)
	ON_WM_SIZE()
...
// 初始化显示字符
void CXDMailInfo::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","MI_UI_MailInfo"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","MI_UI_MailInfo"));
	tbcResult.InsertItem(2,theApp.operXD.ReadValue("MXD","IL_UI_ItemAttr"));
}

// 自动改变大小
void CXDMailInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDMailInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDMailInfo::OnBnClickedSearch()
{
	try
	{
...
// 翻页查询基本资料信息
void CXDMailInfo::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDMailInfo::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDMailInfo::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDMailInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
void CXDMailInfo::ResultInfo()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_MailInfo_Query);//邮件信息
...
// 翻页进行查询
void CXDMailInfo::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 单击结果资料信息保存当前行号
void CXDMailInfo::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击结果资料信息进行查询
void CXDMailInfo::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 1)
	{
		if(listResult.GetItemCount() > 0)
...
//翻页查询详细资料信息
void CXDMailInfo::OnCbnSelchangeCmbdetail()
{
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDMailInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDMailInfo::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDMailInfo::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDMailInfo::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDMailInfo::ShowResultInfo(int m_cmdShow)
{
...
//详细资料信息页面
void CXDMailInfo::ShowDetailInfo(int m_cmdShow)
{
...
//单击基本资料信息的列名进行排序
void CXDMailInfo::OnLvnColumnclickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listBasic,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CXDMailInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listResult,pNMHDR,pResult);
}

//单击详细信息的列名进行排序
void CXDMailInfo::OnLvnColumnclickListdetail(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listDetail,pNMHDR,pResult);
}

//获取一列中所有相同TagName的字符串
CString CXDMailInfo::GetFullItemName(CListCtrl* listCtrlS, int nIndex, CString ColName)
{
	CString ItemInfo = "";
	CString perItem = "";
	CString rColName = "";
