...
IMPLEMENT_DYNAMIC(CXDUserGuild, CDialog)
CXDUserGuild::CXDUserGuild(CWnd* pParent /*=NULL*/)
	: CDialog(CXDUserGuild::IDD, pParent)
{
}

CXDUserGuild::~CXDUserGuild()
{
}

void CXDUserGuild::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDUserGuild, CDialog)
	ON_WM_SIZE()
...
// 初始化对话框
BOOL CXDUserGuild::OnInitDialog()
{
...
// 初始化显示字符
void CXDUserGuild::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","UG_UI_UserGuild"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","UG_UI_UserGuildBasic"));
	tbcResult.InsertItem(2,theApp.operXD.ReadValue("MXD","UG_UI_UserGuildItem"));
}

// 自动改变大小
void CXDUserGuild::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDUserGuild::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDUserGuild::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDUserGuild::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDUserGuild::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDUserGuild::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDUserGuild::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			if(tbcResult.GetCurSel()==1)
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_PlayerGuildBaseInfo_Query);//玩家公会基本信息
			else if(tbcResult.GetCurSel()==2)	
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_PlayerGuildItemInfo_Query);//玩家公会物品信息
...
// 翻页进行查询
void CXDUserGuild::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDUserGuild::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDUserGuild::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDUserGuild::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDUserGuild::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDUserGuild::ShowResultInfo(int m_cmdShow)
{
