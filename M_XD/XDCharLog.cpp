...
IMPLEMENT_DYNAMIC(CXDCharLog, CDialog)
CXDCharLog::CXDCharLog(CWnd* pParent /*=NULL*/)
	: CDialog(CXDCharLog::IDD, pParent)
{
}

CXDCharLog::~CXDCharLog()
{
}

void CXDCharLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDCharLog, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
...
// 初始化显示字符
void CXDCharLog::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","CL_UI_CharLog"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","CL_UI_CharLog"));

	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_Upgrade"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_Create"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_LearnSkill"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_GetQuest"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_FinishQuest"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","CL_UI_FBSearch"));
}


// 自动改变大小
void CXDCharLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDCharLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDCharLog::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDCharLog::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDCharLog::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击基本资料信息进行查询
void CXDCharLog::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDCharLog::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CXDCharLog::OnBnClickedBtnlog()
{
	pageResult = false;
	cmbResult.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
...
void CXDCharLog::ResultLog()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_Upgrade"))
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_RoleUpgrade_Log);//角色升级
			else if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_Create"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_RoleCreate_Log);//角色创建
			else if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_LearnSkill"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_RoleSkill_Log);//角色技能学习
			else if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_GetQuest"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_ReceiveTask_Log);//领取任务
			else if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_FinishQuest"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_CompleteTask_Log);//完成任务
			else if(LogType==theApp.operXD.ReadValue("MXD","CL_UI_FBSearch"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_Copy_Log);//副本查询
...
// 翻页进行查询
void CXDCharLog::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDCharLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDCharLog::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDCharLog::PreTranslateMessage(MSG* pMsg)
{
...
//单击基本资料信息的列名进行排序
void CXDCharLog::OnLvnColumnclickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listBasic,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CXDCharLog::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
