...
void CNotice::OnBtdelete()
{
 //MessageBox("tt"); 
	...
	sprintf(message,"%s%s%s","你所选择的公告ID为:",NoticeIDString," 是否确定要删除所选择公告");
	...
	socketdata.AddTFLV(CEnumCore::TagName::XDJG_TaskID,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//公告ID
	int midlength = socketdata.EnBody();
	if(theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_BoardTasker_Del))//发送编辑公告的请求
	{
...
BOOL CNotice::OnInitDialog()
{
   ...
   ishaveselect=false;
	...
	cmbStatus.AddString(theApp.operXD.ReadValue("MXD","NI_UI_UnSend"));//未发送
	cmbStatus.AddString(theApp.operXD.ReadValue("MXD","NI_UI_Sended"));//已发送
	...
	listResult.InsertColumn(0,theApp.operXD.ReadValue("MXD","NI_UI_ServerList"),LVCFMT_CENTER,150,-1);
...
void CNotice::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","NI_UI_NoticeInfo"));
	SetDlgItemText(IDC_CHECKSELECTALL,theApp.operXD.ReadValue("MXD","NI_UI_SelectAll"));
	SetDlgItemText(IDC_BTNNOTICEINFO,theApp.operXD.ReadValue("MXD","NI_UI_NoticeCheck"));
	SetDlgItemText(IDC_LBLHINT,theApp.operXD.ReadValue("MXD","NI_UI_NoticeHint"));
	SetDlgItemText(IDC_LBLPAGE,theApp.operXD.ReadValue("MXD","BU_UI_LblPage"));
	SetDlgItemText(IDC_LBLSTARTTIME,theApp.operXD.ReadValue("MXD","NI_UI_StartTime"));
	SetDlgItemText(IDC_LBLENDTIME,theApp.operXD.ReadValue("MXD","NI_UI_EndTime"));
	SetDlgItemText(IDC_LBLINTERVAL,theApp.operXD.ReadValue("MXD","NI_UI_TimeInterval"));
	SetDlgItemText(IDC_LBLTIME,theApp.operXD.ReadValue("MXD","NI_UI_Minite"));	
	SetDlgItemText(IDC_LBLCONTENT,theApp.operXD.ReadValue("MXD","NI_UI_SendContent"));
	SetDlgItemText(IDC_CHECKIMME,theApp.operXD.ReadValue("MXD","NI_UI_SendImme"));
	SetDlgItemText(IDC_LBLSTATUS,theApp.operXD.ReadValue("MXD","NI_UI_NoticeStatus"));
	SetDlgItemText(IDBTNNOTICE,theApp.operXD.ReadValue("MXD","NI_UI_AddNotice"));
	SetDlgItemText(IDBTNRESET,theApp.operXD.ReadValue("MXD","Reset"));
}
	...
	if(GetKeyState(VK_CONTROL)&0x8000)
	{
		...
		{				
			if(pMsg->wParam == 'C')
			{
				...
				{
					theApp.operXD.CopyListInfo(&listNotice);				
				}
			}
...
void CNotice::NoticeInfo()
{
	...
	{
		char strInt[10];
		int index = theApp.operXD.ReadIntValue("MXD","index");//查看第一页的记录
		int iPageSize = theApp.operXD.ReadIntValue("MXD","pageSize");//每页有50条记录
		CSocketData socketdata;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_BoardTasker_Query))//发送查询公告信息的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operXD.ReadIntValue("MXD","Send");//当前的状态为发送查询请求
		}
	...
	{
		MessageBox(theApp.operXD.ReadValue("MXD","UIC_Error_NoticeInfo"),theApp.operXD.ReadValue("MXD","Error"),0);
	}	
}
...
void CNotice::OnNMDblclkListnotice(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	try
	{
		...
		if(listNotice.GetItemCount() > 0)//如果公告列表中当前有记录
		{
			...
			if(nItem >= 0)
			{
				NoticeID = atoi(theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_TaskID")));//获取公告ID
				CString startTime = theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_BeginTime"));//获取开始时间
				CString endTime = theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_EndTime"));//获取结束时间
				...
				SetDlgItemText(IDC_EDITINTERVAL,theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_Interval")));//获取时间间隔并设置
				SetDlgItemText(IDC_EDITCONTENT,theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_BoardMessage")));//获取公告内容并设置
				ServerIP = theApp.operXD.GetValueByColName(&listNotice, nItem, theApp.operXD.ReadValue("MXD","XDJG_ServerName"));//获取服务器IP
				SetDlgItemText(IDBTNNOTICE,theApp.operXD.ReadValue("MXD","NI_UI_EditNotice"));
...
void CNotice::OnBnClickedBtnnotice()
{
	try
	{
		...
		if(NoticeOp == theApp.operXD.ReadValue("MXD","NI_UI_AddNotice"))//如果按钮为添加公告
		{
			AddNotice();//添加公告
		}
		else if(NoticeOp == theApp.operXD.ReadValue("MXD","NI_UI_EditNotice"))//如果按钮为编辑公告
		{
			EditNotice();//编辑公告
		}
	}
...
{;	
	...
	{		
		...
		{
			MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_ServerListNull"),theApp.operXD.ReadValue("MXD","Warn"),0);
			return;
		}
		...
		{
			MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_ContentNotNull"),theApp.operXD.ReadValue("MXD","Warn"),0);
			return;
		}
		//原有对字符最大值限制，现在取消
		//if(strlen(NoticeContent) >= 256)//公告内容不能超过256个字符
		//{
		//	MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_ContentTooLong"),theApp.operXD.ReadValue("MXD","Warn"),0);
		//	return;
		//}
		...
		else//如果不是立即发送
		{
			...
			{
				MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_StartTime"),theApp.operXD.ReadValue("MXD","Warn"),0);
				return;
			}
			if(toTime <= beginTime)//结束时间不能小于开始时间
			{
				MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_TimeSpan"),theApp.operXD.ReadValue("MXD","Warn"),0);
				return;
			}
			if(interval <= 0)//时间间隔必须大于0
			{
				MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_IntervalNull"),theApp.operXD.ReadValue("MXD","Warn"),0);
				return;
			}
						if(interval <= 1)//时间间隔必须大于0
			{
				MessageBox("时间间隔不能小于1分钟",theApp.operXD.ReadValue("MXD","Warn"),0);
				return;
			}
			CTimeSpan sTimeSpan = toTime - beginTime;
			if(interval >= sTimeSpan.GetTotalMinutes())//时间间隔必须在时间范围之内
			{
				MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_IntervalLong"),theApp.operXD.ReadValue("MXD","Warn"),0);
				return;
			}
		}

		CString strMsg = theApp.operXD.ReadValue("MXD","NI_Hint_AddNotice"); 
		...
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_BeginTime,CEnumCore::TagFormat::TLV_STRING,strlen(strBeginTime),(unsigned char*)mIni.wcharToChar(strBeginTime));//开始时间
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_EndTime,CEnumCore::TagFormat::TLV_STRING,strlen(strEndTime),(unsigned char*)mIni.wcharToChar(strEndTime));//结束时间

		_itoa(interval,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_Interval,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//时间间隔
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_BoardMessage,CEnumCore::TagFormat::TLV_STRING,strlen(NoticeContent),(unsigned char *)mIni.wcharToChar(NoticeContent));//公告内容
		int userByID = theApp.UserByID;
		_itoa(userByID,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::UserByID,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//操作员ID
		...
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_ServerIP,CEnumCore::TagFormat::TLV_STRING,strlen(strServerIP),(unsigned char *)&strServerIP);//服务器IP
		socketdata.AddTFLV(CEnumCore::TagName::XDJG_ServerName,CEnumCore::TagFormat::TLV_STRING,strlen(ServerName),(unsigned char *)mIni.wcharToChar(ServerName));//服务器IP)
        
		int midlength = socketdata.EnBody();
		if(theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_BoardTasker_Add))//发送添加公告的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operXD.ReadIntValue("MXD","Send");//当前的状态为发送查询请求
		}
...
{	
	...
	{		
		...
		{
			MessageBox(theApp.operXD.ReadValue("MXD","NI_Warn_StatusNotNull"),theApp.operXD.ReadValue("MXD","Warn"),0);
			return;
		}
		...
		CString strMsg = theApp.operXD.ReadValue("MXD","NI_Hint_EditNotice"); 
		...
		int midlength = socketdata.EnBody();
		if(theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_BoardTasker_Update))//发送编辑公告的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operXD.ReadIntValue("MXD","Send");//当前的状态为发送查询请求
		}
...
LRESULT CNotice::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	dlgStatus = theApp.operXD.ReadIntValue("MXD","Receive");//当前的状态为接收到返回信息
	...
	case CEnumCore::Message_Tag_ID::SERVERINFO_IP_QUERY_RESP://如果接收到的是服务器列表返回信息
		{
			...
			{
				MessageBox((LPCSTR)&m_tflv.lpdata,theApp.operXD.ReadValue("MXD","Error"),0);
				socketdata.Destroy();
				return false;
			}
		...
		break;
	case CEnumCore::Message_Tag_ID::XDJG_BoardTasker_Query_Resp://如果接收到的是公告列表请求的返回信息
		{
			
			if(pageInfo)//已经可以翻页
			{
				theApp.operXD.BuildDataTable(&socketdata,&listNotice,&pageCount);//构造公告列表信息
			}
			else
			{
				theApp.operXD.BuildDataTable(&socketdata,&listNotice,&pageCount);//构造公告列表信息
		...
		break;
	case CEnumCore::Message_Tag_ID::XDJG_BoardTasker_Add_Resp://如果接收到的是添加公告请求的返回信息
		{
			CGlobalStruct::TFLV m_tflv=socketdata.getTLVBynIndex(1);
			CString result = (LPCTSTR)&m_tflv.lpdata;
			theApp.operXD.ShowResult(result);//显示添加公告的结果
		...
		break;
	case CEnumCore::Message_Tag_ID::XDJG_BoardTasker_Update_Resp://如果接收到的是编辑公告请求的返回信息
		{
			CGlobalStruct::TFLV m_tflv=socketdata.getTLVBynIndex(1);
			CString result = (LPCTSTR)&m_tflv.lpdata;
			theApp.operXD.ShowResult(result);//显示编辑公告的结果
			NoticeInfo();//查询所有的公告信息
		...
		break;
	case CEnumCore::Message_Tag_ID::XDJG_BoardTasker_Del_Resp://如果接收到的是删除公告请求的返回信息
		{
			CGlobalStruct::TFLV m_tflv=socketdata.getTLVBynIndex(1);
			CString result = (LPCTSTR)&m_tflv.lpdata;
			theApp.operXD.ShowResult(result);//显示编辑公告的结果
...
{
	//OperationPAL operXD;
	theApp.operXD.SortListInfo(&listNotice,pNMHDR,pResult);
}
