...
IMPLEMENT_DYNAMIC(CXDGuildInfo, CDialog)
CXDGuildInfo::CXDGuildInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CXDGuildInfo::IDD, pParent)
{
}

CXDGuildInfo::~CXDGuildInfo()
{
}

void CXDGuildInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDGuildInfo, CDialog)
	ON_WM_SIZE()
...
// 初始化对话框
BOOL CXDGuildInfo::OnInitDialog()
{
...
// 初始化显示字符
void CXDGuildInfo::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","GI_UI_GuildInfo"));
	...
	SetDlgItemText(IDC_LBLGUILD,theApp.operXD.ReadValue("MXD","GI_UI_GuildName"));
	...
	//tbcResult.InsertItem(2,theApp.operXD.ReadValue("MXD","GI_UI_GuildStorage"));
	//tbcResult.InsertItem(3,theApp.operXD.ReadValue("MXD","GI_UI_GuildWage"));
	//tbcResult.InsertItem(4,theApp.operXD.ReadValue("MXD","GI_UI_GuildExp"));
}


// 自动改变大小
void CXDGuildInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDGuildInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDGuildInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDGuildInfo::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDGuildInfo::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDGuildInfo::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDGuildInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
// 翻页进行查询
void CXDGuildInfo::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDGuildInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDGuildInfo::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDGuildInfo::PreTranslateMessage(MSG* pMsg)
{
