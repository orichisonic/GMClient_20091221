...
IMPLEMENT_DYNAMIC(CXDMailLog, CDialog)
CXDMailLog::CXDMailLog(CWnd* pParent /*=NULL*/)
	: CDialog(CXDMailLog::IDD, pParent)
{
}

CXDMailLog::~CXDMailLog()
{
}

void CXDMailLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDMailLog, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
...
// 初始化显示字符
void CXDMailLog::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","ML_UI_MailLog"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","ML_UI_MailLog"));
	tbcResult.InsertItem(2,theApp.operXD.ReadValue("MXD","IL_UI_ItemAttr"));

	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","ML_UI_MailSend"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","ML_UI_MailRecv"));
}


// 自动改变大小
void CXDMailLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDMailLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDMailLog::OnBnClickedSearch()
{
	try
	{
...
// 翻页查询基本资料信息
void CXDMailLog::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDMailLog::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击基本资料信息进行查询
void CXDMailLog::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDMailLog::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CXDMailLog::OnBnClickedBtnlog()
{
	pageResult = false;
...
void CXDMailLog::ResultLog()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(LogType==theApp.operXD.ReadValue("MXD","ML_UI_MailSend"))
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_SendMail_Log);//发送邮件
			else if(LogType==theApp.operXD.ReadValue("MXD","ML_UI_MailRecv"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_ReceiveMail_Log);//收取邮件
...
// 翻页进行查询
void CXDMailLog::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 单击结果资料信息保存当前行号
void CXDMailLog::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击结果资料信息进行查询
void CXDMailLog::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 1)
	{
		if(listResult.GetItemCount() > 0)
...
//翻页查询详细资料信息
void CXDMailLog::OnCbnSelchangeCmbdetail()
{
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDMailLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDMailLog::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDMailLog::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDMailLog::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDMailLog::ShowResultInfo(int m_cmdShow)
{
...
//详细资料信息页面
void CXDMailLog::ShowDetailInfo(int m_cmdShow)
{
...
//单击基本资料信息的列名进行排序
void CXDMailLog::OnLvnColumnclickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listBasic,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CXDMailLog::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listResult,pNMHDR,pResult);
}

//单击详细信息的列名进行排序
void CXDMailLog::OnLvnColumnclickListdetail(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listDetail,pNMHDR,pResult);
}

//获取一列中所有相同TagName的字符串
CString CXDMailLog::GetFullItemName(CListCtrl* listCtrlS, int nIndex, CString ColName)
{
	CString ItemInfo = "";
	CString perItem = "";
	CString rColName = "";
