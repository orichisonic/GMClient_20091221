...
IMPLEMENT_DYNAMIC(CXDGuildLog, CDialog)
CXDGuildLog::CXDGuildLog(CWnd* pParent /*=NULL*/)
	: CDialog(CXDGuildLog::IDD, pParent)
{
}

CXDGuildLog::~CXDGuildLog()
{
}

void CXDGuildLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDGuildLog, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
...
// 初始化显示字符
void CXDGuildLog::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","GL_UI_GuildLog"));
	...
	SetDlgItemText(IDC_LBLNAME,theApp.operXD.ReadValue("MXD","GI_UI_GuildName"));
	...
	tbcResult.InsertItem(1,theApp.operXD.ReadValue("MXD","GL_UI_GuildLog"));

	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","GL_UI_Create"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","GL_UI_Disband"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","GL_UI_AddMember"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","GL_UI_DelMember"));
	cmbLogType.AddString(theApp.operXD.ReadValue("MXD","GL_UI_ChangePost"));
}


// 自动改变大小
void CXDGuildLog::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDGuildLog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDGuildLog::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDGuildLog::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDGuildLog::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
...
// 双击基本资料信息进行查询
void CXDGuildLog::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDGuildLog::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
...
//查询日志信息
void CXDGuildLog::OnBnClickedBtnlog()
{
	pageResult = false;
	cmbResult.ResetContent();
	while(listResult.DeleteColumn(0)){}// 清除结果列表中的所有内容
...
void CXDGuildLog::ResultLog()
{
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			int midlength = socketdata.EnBody();
			if(LogType==theApp.operXD.ReadValue("MXD","GL_UI_Create"))
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildCreate_Log);//公会成立
			else if(LogType==theApp.operXD.ReadValue("MXD","GL_UI_Disband"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildDismiss_Log);//公会解散
			else if(LogType==theApp.operXD.ReadValue("MXD","GL_UI_AddMember"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildAddMember_Log);//公会添加成员
			else if(LogType==theApp.operXD.ReadValue("MXD","GL_UI_DelMember"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildMemberLeaved_Log);//公会成员离开
			else if(LogType==theApp.operXD.ReadValue("MXD","GL_UI_ChangePost"))			
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildPostChanged_Log);//公会职务变更
...
// 翻页进行查询
void CXDGuildLog::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDGuildLog::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
	...
	case CEnumCore::Message_Tag_ID::SERVERINFO_IP_QUERY_RESP://查询服务器列表信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::XDJG_GuildBaseInfo_Query_Resp://查询基本资料信息的回应
		{
...
// 清空页面
void CXDGuildLog::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDGuildLog::PreTranslateMessage(MSG* pMsg)
{
