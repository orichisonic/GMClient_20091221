...
IMPLEMENT_DYNAMIC(CXDItemInfo, CDialog)
CXDItemInfo::CXDItemInfo(CWnd* pParent /*=NULL*/)
	: CDialog(CXDItemInfo::IDD, pParent)
{
}

CXDItemInfo::~CXDItemInfo()
{
}

void CXDItemInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXDItemInfo, CDialog)
	ON_WM_SIZE()
...
// 初始化显示字符
void CXDItemInfo::InitUI()
{
	SetWindowText(theApp.operXD.ReadValue("MXD","IC_UI_ITEMCHECK"));
	...
	tbcResult.InsertItem(4,theApp.operXD.ReadValue("MXD","IC_UI_GuildItem"));
	tbcResult.InsertItem(5,theApp.operXD.ReadValue("MXD","IC_UI_KKItem"));
}

// 自动改变大小
void CXDItemInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CXDItemInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// 查询基本资料信息
void CXDItemInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
// 翻页查询基本资料信息
void CXDItemInfo::OnCbnSelchangeCmbbasic()
{	
	try
	{	
...
// 单击基本资料信息保存当前行号
void CXDItemInfo::OnNMClickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击基本资料信息进行查询
void CXDItemInfo::OnNMDblclkListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	if(tbcResult.GetCurSel() == 0)
	{
		if(listBasic.GetItemCount() > 0)
...
// 切换选项卡进行查询
void CXDItemInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{		
	...
	{		
		...
		if((listBasic.GetItemCount() > 0) && (nItem>=0))
		{
			...
			if(tbcResult.GetCurSel()==1)
				...
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_GuildStorage_Query);//公会仓库信息
			else if(tbcResult.GetCurSel()==5)	
				theApp.operXD.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::XDJG_KingkongStorage_Query);//金刚仓库信息
...
// 翻页进行查询
void CXDItemInfo::OnCbnSelchangeCmbresult()
{	
	try
	{
		if(pageResult)
...
// 接收返回信息
LRESULT CXDItemInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{	
...
// 清空页面
void CXDItemInfo::OnBnClickedCancel()
{
...
//全选、复制
BOOL CXDItemInfo::PreTranslateMessage(MSG* pMsg)
{
...
//基本资料信息页面
void CXDItemInfo::ShowBasicInfo(int m_cmdShow)
{
...
//结果资料信息页面
void CXDItemInfo::ShowResultInfo(int m_cmdShow)
{
...
//单击基本资料信息的列名进行排序
void CXDItemInfo::OnLvnColumnclickListbasic(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operXD.SortListInfo(&listBasic,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CXDItemInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
