...
IMPLEMENT_DYNAMIC(CGTOWNRoomInfo, CDialog)
CGTOWNRoomInfo::CGTOWNRoomInfo(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNRoomInfo::IDD, pParent)
{
}

CGTOWNRoomInfo::~CGTOWNRoomInfo()
{
}

void CGTOWNRoomInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNRoomInfo, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNRoomInfo 消息处理程序


// 初始化对话框
BOOL CGTOWNRoomInfo::OnInitDialog()
{
...
//初始化显示字符
void CGTOWNRoomInfo::InitUI()
{
	SetWindowText(theApp.operGTOWN.ReadValue("MGTOWN","RI_UI_RoomInfo"));
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","RI_UI_tpgRoomInfo"));//玩家房屋信息
}



//处理快捷键响应函数
BOOL CGTOWNRoomInfo::PreTranslateMessage(MSG* pMsg)
{
...
//自动改变大小
void CGTOWNRoomInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNRoomInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNRoomInfo::OnBnClickedSearch()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
//翻页查询玩家资料信息
void CGTOWNRoomInfo::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNRoomInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNRoomInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNRoomInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
void CGTOWNRoomInfo::RoomInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_RoomInfo_Query))//人物房屋信息
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
	...
	{
		MessageBox(theApp.operGTOWN.ReadValue("MGTOWN","UIC_Error_Room"),theApp.operGTOWN.ReadValue("MGTOWN","Error"),0);
	}	
}

//切换翻页组合框
void CGTOWNRoomInfo::OnCbnSelchangeCmbpage()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_RoomInfo_Query))//发送人物房屋信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
//接受返回信息
LRESULT CGTOWNRoomInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//显示玩家信息
void CGTOWNRoomInfo::ShowUserInfo()
{
...
//显示其他信息
void CGTOWNRoomInfo::ShowOtherInfo()
{
...
//单击用户资料信息的列名进行排序
void CGTOWNRoomInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNRoomInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//使用关闭按钮
void CGTOWNRoomInfo::OnBnClickedCancel()
{
