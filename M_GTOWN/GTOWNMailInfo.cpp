...
IMPLEMENT_DYNAMIC(CGTOWNMailInfo, CDialog)
CGTOWNMailInfo::CGTOWNMailInfo(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNMailInfo::IDD, pParent)
{
}

CGTOWNMailInfo::~CGTOWNMailInfo()
{
}

void CGTOWNMailInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNMailInfo, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNMailInfo 消息处理程序

//初始化对话框
BOOL CGTOWNMailInfo::OnInitDialog()
{
...
// 初始化显示字符
void CGTOWNMailInfo::InitUI()
{
	SetWindowText(theApp.operGTOWN.ReadValue("MGTOWN","MI_UI_MailInfo"));
	...
	SetDlgItemText(IDC_LBLSTARTTIME,theApp.operGTOWN.ReadValue("MGTOWN","NI_UI_StartTime"));//开始时间
	SetDlgItemText(IDC_LBLENDTIME,theApp.operGTOWN.ReadValue("MGTOWN","NI_UI_EndTime"));//结束时间

	tbcResult.InsertItem(0,theApp.operGTOWN.ReadValue("MGTOWN","UIC_UI_tpgUserInfo"));//玩家基本信息
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","MI_UI_tpgMailInfo"));//玩家邮件信息
}



//处理快捷键响应函数
BOOL CGTOWNMailInfo::PreTranslateMessage(MSG* pMsg)
{
...
// 自动改变大小
void CGTOWNMailInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNMailInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNMailInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
//翻页查询玩家资料信息
void CGTOWNMailInfo::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNMailInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNMailInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNMailInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
void CGTOWNMailInfo::MailInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_MailInfo_Query))//邮件信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//切换翻页组合框
void CGTOWNMailInfo::OnCbnSelchangeCmbpage()
{
	try
	{	
			...
			case 1:
				{
					...
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_MailInfo_Query))//发送邮件信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CGTOWNMailInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//单击用户资料信息的列名进行排序
void CGTOWNMailInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNMailInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//显示玩家信息
void CGTOWNMailInfo::ShowUserInfo()
{
...
//显示其他信息
void CGTOWNMailInfo::ShowOtherInfo()
{
	tbcResult.SetCurSel(1);
...
//使用关闭按钮
void CGTOWNMailInfo::OnBnClickedCancel()
{
