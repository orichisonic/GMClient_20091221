...
IMPLEMENT_DYNAMIC(CGTOWNBanPlayer, CDialog)
CGTOWNBanPlayer::CGTOWNBanPlayer(CWnd* pParent /*=NULL*/)
	: CDialog(CGTOWNBanPlayer::IDD, pParent)
{
}

CGTOWNBanPlayer::~CGTOWNBanPlayer()
{
}

void CGTOWNBanPlayer::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNBanPlayer, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNBanPlayer 消息处理程序

BOOL CGTOWNBanPlayer::OnInitDialog()
{
...
void CGTOWNBanPlayer::InitUI()
{
	SetWindowText(theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_BanPlayer"));
	...
	SetDlgItemText(IDC_LBLHINT,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_LblHint"));	
	...
	SetDlgItemText(IDC_LBLBANREASON,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_reason"));
	SetDlgItemText(IDC_BTNBAN,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_TpgOpen"));
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_TpgClose"));//封停账号
	tbcResult.InsertItem(2,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_TpgList"));//停封账号列表
	tbcResult.InsertItem(3,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_TpgOpen"));//解封操作
	tbcResult.InsertItem(4,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_Tpgquary"));//已封停帐号查询
}

//处理快捷键响应
BOOL CGTOWNBanPlayer::PreTranslateMessage(MSG* pMsg)
{
...
//自动调节大小
void CGTOWNBanPlayer::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNBanPlayer::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNBanPlayer::OnBnClickedOk()
{
	try
	{
		CIni mIni;
...
//翻页查询玩家资料信息
void CGTOWNBanPlayer::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 查询所有被封停帐号列表信息
void CGTOWNBanPlayer::AllBanUserList()
{
	try
	{				
		CIni mIni;
...
// 翻页进行查询//所有被封停的帐号
void CGTOWNBanPlayer::OnCbnSelchangeCmbpage()
{
	try
	{		
		if(pageInfo)
...
// 单击用户列表保存当前行号
void CGTOWNBanPlayer::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击用户列表切换界面
void CGTOWNBanPlayer::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listUser.GetItemCount() != 0)
...
void CGTOWNBanPlayer::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CGTOWNBanPlayer::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
//切换tabCtrl选项卡
void CGTOWNBanPlayer::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	CIni mIni;
...
//封停或解封帐号
void CGTOWNBanPlayer::OnBnClickedBtnban()
{
	try
	{
		CIni mIni;		
		...
		{
			MessageBox(theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_AccountNull"),theApp.operGTOWN.ReadValue("MGTOWN","Warn"),0);
			return;
		}				
		...
		{	
			...
			{
				MessageBox(theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_ReasonTooLong"),theApp.operGTOWN.ReadValue("MGTOWN","Warn"),0);
				return;
			}
		}
		...
		if(tbcResult.GetCurSel() == 1)
		{
			...
			if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_USER_STOP))//发送封停玩家帐号的请求
			{
				::EnableWindow(this->GetParent()->m_hWnd,false);
		        dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
			}
		...
		else if(tbcResult.GetCurSel() == 3)
		{
			...
			if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_USER_STOP))//发送解封玩家帐号的请求
			{
				::EnableWindow(this->GetParent()->m_hWnd,false);
		        dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
			}
		...
		{
			int midlength = socketdata.EnBody();
			if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_BanUser_Query))//发送查询玩家帐号有没有被封停的请求
			{
				::EnableWindow(this->GetParent()->m_hWnd,false);
		        dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
			}
...
//重置信息
void CGTOWNBanPlayer::OnBnClickedBtnreset()
{
	SetDlgItemText(IDC_EDITBANREASON,"");
...
//接受返回信息
LRESULT CGTOWNBanPlayer::OnMessageReceive(WPARAM wParam,LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::GT_UserInfo_Query_Resp://玩家资料查询的反应
		...
		break;
		//封停玩家帐号的回应
	case CEnumCore::Message_Tag_ID::GT_USER_STOP_ACK:
		{
		...
		break;
		//查询所有被封停帐号的回应
	case CEnumCore::Message_Tag_ID::GT_AllBanUser_Query_Resp:
		{
			if(pageInfo)
			{
		...
		break;
	case CEnumCore::Message_Tag_ID::GT_BanUser_Query_Resp:
		{
...
//显示玩家资料信息
void CGTOWNBanPlayer::showUserInfo()
{
...
//显示封停帐号
void CGTOWNBanPlayer::showBanPlayer()
{
	...
	SetDlgItemText(IDC_LBLBANREASON,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_reason"));//封停原因
	SetDlgItemText(IDC_BTNBAN,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_BtnPost"));//"BU_UI_TpgOpen"));//帐号封停
	txtAccount.SetReadOnly(true);//玩家帐号的CEdit对象设置为只读属性
}

//显示封停帐号列表
void CGTOWNBanPlayer::showBanList()
{
...
//显示解封帐号
void CGTOWNBanPlayer::showUnbindPlayer()
{
	...
	SetDlgItemText(IDC_LBLBANREASON,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_UnBind"));//解封原因
	SetDlgItemText(IDC_BTNBAN,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_BtnSave"));//解封帐号

	txtAccount.SetReadOnly(true);//玩家帐号的CEdit对象设置为只读属性
}

//查询帐号有没有封停
void CGTOWNBanPlayer::showSearchPlayer()
{
...
//单击用户资料信息的列名进行排序
void CGTOWNBanPlayer::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNBanPlayer::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//使用关闭按钮
void CGTOWNBanPlayer::OnBnClickedCancel()
{
