...
IMPLEMENT_DYNAMIC(CGTOWNUserInfoPart, CDialog)
CGTOWNUserInfoPart::CGTOWNUserInfoPart(CWnd* pParent /*=NULL*/)
	: CDialog(CGTOWNUserInfoPart::IDD, pParent)
{
}

CGTOWNUserInfoPart::~CGTOWNUserInfoPart()
{
}

void CGTOWNUserInfoPart::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNUserInfoPart, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNUserInfoPart 消息处理程序

BOOL CGTOWNUserInfoPart::OnInitDialog()
{
...
void CGTOWNUserInfoPart::InitUI()
{
...
//	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","UIC_UI_tpgDetailInfo"));//用户详细资料信息
//	tbcResult.InsertItem(2,theApp.operGTOWN.ReadValue("MGTOWN","UIC_UI_tpgFamilyInfo"));//角色家族信息
//	tbcResult.InsertItem(3,theApp.operGTOWN.ReadValue("MGTOWN","UIC_UI_tpgSentiInfo"));//角色情侣信息
//	tbcResult.InsertItem(4,theApp.operGTOWN.ReadValue("MGTOWN","UIC_UI_tpgMarrigeInfo"));//角色结婚信息
}

//处理快捷键响应函数
BOOL CGTOWNUserInfoPart::PreTranslateMessage(MSG* pMsg)
{
...
//自动调节大小
void CGTOWNUserInfoPart::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNUserInfoPart::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNUserInfoPart::OnBnClickedSearch()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
		CIni mIni;
		ShowUserInfo();//显示玩家资料信息的界面
		...
		if((Account == "") && (Name == ""))//玩家帐号和玩家角色名不能同时为空
		{
...
// 翻页进行查询
void CGTOWNUserInfoPart::OnCbnSelchangeCmbpage()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNUserInfoPart::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNUserInfoPart::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNUserInfoPart::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
//玩家详细资料查询
void CGTOWNUserInfoPart::DetailInfo()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_User_DetailInfo_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//角色家族信息
void CGTOWNUserInfoPart::FamilyInfo()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_User_FamilyInfo_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//情侣信息查询
void CGTOWNUserInfoPart::SentiInfo()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_User_SentiInfo_Query))//发送情侣信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//结婚信息查询
void CGTOWNUserInfoPart::MarrigeInfo()
{
	try
	{		
		CIni mIni;
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_User_MarrigeInfo_Query))//发送结婚信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
// 接收返回信息
LRESULT CGTOWNUserInfoPart::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//单击用户资料信息的列名进行排序
void CGTOWNUserInfoPart::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//单击结果信息的列名进行排序
void CGTOWNUserInfoPart::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//关闭对话框
void CGTOWNUserInfoPart::OnClose()
{
...
void CGTOWNUserInfoPart::OnBnClickedCancel()
{
...
// 显示角色资料信息
void CGTOWNUserInfoPart::ShowUserInfo()
{
...
// 显示其它信息
void CGTOWNUserInfoPart::ShowOtherInfo()
{
