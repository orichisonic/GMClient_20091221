...
// CGTOWNRoleManage 对话框

IMPLEMENT_DYNAMIC(CGTOWNRoleManage, CDialog)
CGTOWNRoleManage::CGTOWNRoleManage(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNRoleManage::IDD, pParent)
{
}

CGTOWNRoleManage::~CGTOWNRoleManage()
{
}

void CGTOWNRoleManage::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNRoleManage, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	...
	ON_BN_CLICKED(IDC_BTNMOVEUUSER, OnBnClickedBtnmoveuuser)
	ON_BN_CLICKED(IDC_BTNRESETSTATUS, OnBnClickedBtnresetstatus)
	ON_BN_CLICKED(IDC_BTNRESET, OnBnClickedBtnreset)
	ON_BN_CLICKED(IDC_BTNCLEARNAME, OnBnClickedBtnclearname)
END_MESSAGE_MAP()


// CGTOWNRoleManage 消息处理程序

//初始化对话框
BOOL CGTOWNRoleManage::OnInitDialog()
{
...
void CGTOWNRoleManage::InitUI()
{
	...
	SetDlgItemText(IDC_BTNMOVEUUSER,theApp.operGTOWN.ReadValue("MGTOWN","RM_UI_BtnMoveRole"));
	SetDlgItemText(IDC_BTNRESETSTATUS,theApp.operGTOWN.ReadValue("MGTOWN","RM_UI_BtnResetStatus"));
	SetDlgItemText(IDC_BTNRESET,theApp.operGTOWN.ReadValue("MGTOWN","Reset"));
}

//处理快捷键响应函数
BOOL CGTOWNRoleManage::PreTranslateMessage(MSG* pMsg)
{
...
//自动改变大小
void CGTOWNRoleManage::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNRoleManage::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNRoleManage::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
//翻页查询玩家资料信息
void CGTOWNRoleManage::OnCbnSelchangeCmbpage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNRoleManage::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// 双击玩家资料信息将玩家角色移动至黑屋
void CGTOWNRoleManage::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listUser.GetItemCount() > 0)
...
//将选定玩家角色移至黑屋
void CGTOWNRoleManage::OnBnClickedBtnmoveuuser()
{
	try
	{
		CIni mIni;

		GetDlgItemText(IDC_EDITNICK,UserName);//获取玩家角色名
		...
		if (AfxMessageBox(theApp.operGTOWN.ReadValue("MGTOWN","RM_Hint_MoveRole"), MB_YESNO) == IDNO) 
		{
			return;
		}
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_MOVE_USER))//发送将玩家角色移动至黑屋的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//重设玩家到出生状态
void CGTOWNRoleManage::OnBnClickedBtnresetstatus()
{
	try
	{
		CIni mIni;
		...
		if (AfxMessageBox(theApp.operGTOWN.ReadValue("MGTOWN","RM_Hint_ResetStatus"), MB_YESNO) == IDNO) 
		{
			return;
		}
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_USER_RESET))//发送重设玩家到出生状态的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//重置玩家昵称信息
void CGTOWNRoleManage::OnBnClickedBtnreset()
{
	SetDlgItemText(IDC_EDITNICK, "");
}

// 接收返回信息
LRESULT CGTOWNRoleManage::OnMessageReceive(WPARAM wParam,LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::GT_UserInfo_Query_Resp://玩家资料查询
		...
		break;
	case CEnumCore::Message_Tag_ID::GT_MOVE_USER_ACK://将角色移至黑屋的回应
...
//单击用户资料信息的列名进行排序
void CGTOWNRoleManage::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//选择是否显示相应的控件信息
void CGTOWNRoleManage::ShowOperCtrl(int nCmdShow)
{
	GetDlgItem(IDC_LBLNICK)->ShowWindow(nCmdShow);
	GetDlgItem(IDC_EDITNICK)->ShowWindow(nCmdShow);
	GetDlgItem(IDC_BTNMOVEUUSER)->ShowWindow(nCmdShow);
	GetDlgItem(IDC_BTNRESETSTATUS)->ShowWindow(nCmdShow);
	GetDlgItem(IDC_BTNCLEARNAME)->ShowWindow(nCmdShow);
	GetDlgItem(IDC_BTNRESET)->ShowWindow(nCmdShow);
}

// 关闭对话框
void CGTOWNRoleManage::OnBnClickedCancel()
{
...
void CGTOWNRoleManage::OnBnClickedBtnclearname()
{
	...
	try
	{
		...
		if (AfxMessageBox(theApp.operGTOWN.ReadValue("MGTOWN","RM_Hint_ClearName"), MB_YESNO) == IDNO) 
		{
			return;
		}
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_ClearName_Query))//发送重设玩家到出生状态的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
