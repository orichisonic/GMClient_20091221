...
void CGTOWNFriendTrade::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","FT_UI_tpgFriendTrade"));//朋友买卖中的角色信息
	tbcResult.InsertItem(2,theApp.operGTOWN.ReadValue("MGTOWN","FT_UI_tpgHelotInfo"));//奴隶信息
	tbcResult.InsertItem(3,theApp.operGTOWN.ReadValue("MGTOWN","FT_UI_tpgTradeQueue"));//排行
	tbcResult.InsertItem(4,theApp.operGTOWN.ReadValue("MGTOWN","FT_UI_tpgTradeLog"));//买卖日志
}
...
void CGTOWNFriendTrade::OnBnClickedSearch()
{
	...
	try
	{
		...
		index = theApp.operGTOWN.ReadIntValue("MGTOWN","index");//查询第一页的信息
...
void CGTOWNFriendTrade::TradeQueue()
{
	...
	{		
		...
		socketdata.AddTFLV(CEnumCore::TagName::GT_Type,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//用户ID
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_FriendTrade_Queue))//发送情侣信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNFriendTrade::TradeLog()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_FriendTrade_Log))//发送结婚信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
LRESULT CGTOWNFriendTrade::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::GT_UserInfo_Query_Resp://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::GT_FriendTrade_Queue_Resp://查询玩家资料信息的回应
		{
			if(pageUIInfo)//如果已经能够翻页
			{
...
void CGTOWNFriendTrade::OnCbnSelchangeCmbupage()
{
	try
	{	
		if(pageUIInfo)//如果可以翻页的话
		{
			CIni mIni;

			int index = cmbUIPage.GetCurSel() + 1;//页数为cmbUPage当前的索引值加1，也就是cmbUPage中当前显示的值
			...
			case 1:
				{
					...
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_FriendTrade_Info))//发送邮件信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 2:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_HelotInfo_Query))//发送邮件信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 3:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_FriendTrade_Queue))//发送邮件信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 4:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_FriendTrade_Log))//发送邮件信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
void CGTOWNFriendTrade::InitType(void)
{
	...
	m_cmbType.AddString("身价最贵");
	m_cmbType.AddString("最新被买");
	m_cmbType.AddString("资产最多");
	m_cmbType.AddString("现金最多");
	m_cmbType.AddString("折扣最多");
