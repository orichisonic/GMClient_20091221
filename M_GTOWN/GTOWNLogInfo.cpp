...
IMPLEMENT_DYNAMIC(CGTOWNLogInfo, CDialog)
CGTOWNLogInfo::CGTOWNLogInfo(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNLogInfo::IDD, pParent)
{
}

CGTOWNLogInfo::~CGTOWNLogInfo()
{
}

void CGTOWNLogInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNLogInfo, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNLogInfo 消息处理程序

// 初始化对话框
BOOL CGTOWNLogInfo::OnInitDialog()
{
...
//初始化显示字符
void CGTOWNLogInfo::InitUI()
{
	SetWindowText(theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_LogInfo"));
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_TradeLog"));//交易日志
	tbcResult.InsertItem(2,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_MailLog"));//邮件日志
	tbcResult.InsertItem(3,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_TaskLog"));//任务完成日志
	tbcResult.InsertItem(4,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_UseItemLog"));//使用道具日志（收费道具）
	tbcResult.InsertItem(5,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_GuajiLog"));//挂机奖励日志
	tbcResult.InsertItem(6,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_BugleLog"));//大小喇叭日志
	tbcResult.InsertItem(7,theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_LoginLog"));//账号登录登出日志
}

// 自动改变大小
void CGTOWNLogInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNLogInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//处理快捷键响应
BOOL CGTOWNLogInfo::PreTranslateMessage(MSG* pMsg)
{
...
// 查询玩家资料信息
void CGTOWNLogInfo::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
//翻页查询玩家资料信息
void CGTOWNLogInfo::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNLogInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNLogInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNLogInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
	...
	else
	{
		...
		if((listUser.GetItemCount() > 0) && (nItem >= 0))
		{
			...
			{
				UseItemLog();//使用道具日志（收费道具）
			}
			if(tbcResult.GetCurSel()==5)
			{
				GuajiLog();//挂机奖励日志
			}
			if(tbcResult.GetCurSel()==6)
			{
				BugleLog();//大小喇叭日志
			}
...
void CGTOWNLogInfo::TradeLog()
{
	try
	{
		...
		int LogType=cmbLogType.GetCurSel()+1;
		...
		CSocketData socketdata;	
		socketdata.InitCreate(1,10);//共有五个参数，服务器IP、服务器Name、玩家昵称、页数、每页的记录条数,开始时间，结束时间
		char strInt[10];
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TradeLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::MailLog()
{
	...
	{	
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_MailLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::TaskLog()
{
	try
	{
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TaskLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::UseItemLog()
{
	try
	{
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_UseItemLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::GuajiLog()
{
	try
	{
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_GuajiLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::BugleLog()
{
	try
	{
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_BugleLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNLogInfo::LoginLog()
{
	try
	{
		...
		CSocketData socketdata;	
		socketdata.InitCreate(1,10);//共有九个参数，服务器IP、服务器Name、玩家昵称、玩家帐号、玩家ID、页数、每页的记录条数,开始时间，结束时间
		char strInt[10];
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_LoginLog_Query))
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//切换日志类型进行查询
void CGTOWNLogInfo::OnCbnSelchangeCmblogtype()
{
	try
	{
		CIni mIni;
		...
		case 1:
			{
				...
				if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TradeLog_Query))//交易日志
				{
					::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
					dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
				}
		...
		case 2:
			{
				midlength=socketdata.EnBody();
				if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_MailLog_Query))//邮件日志
				{
					::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
					dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
				}
...
//翻页进行查询
void CGTOWNLogInfo::OnCbnSelchangeCmbpage()
{
	try
	{	
		if(pageInfo)//如果可以翻页的话
		{
			...
			case 3:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TaskLog_Query))//任务完成日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 4:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_UseItemLog_Query))//使用道具日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 5:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_GuajiLog_Query))//挂机奖励日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 6:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_BugleLog_Query))//大小喇叭日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 7:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_LoginLog_Query))//登陆登出日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
// 接收返回信息
LRESULT CGTOWNLogInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//导入日志类型
void CGTOWNLogInfo::LogType()
{
	cmbLogType.ResetContent();
	cmbLogType.AddString(theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_SENDER"));//发送方
	cmbLogType.AddString(theApp.operGTOWN.ReadValue("MGTOWN","LI_UI_RECEIVER"));//接收方
	cmbLogType.SetCurSel(0);
}

// 显示角色资料信息
void CGTOWNLogInfo::ShowUserInfo()
{
...
//显示其它信息
void CGTOWNLogInfo::ShowOtherInfo()
{
...
//单击用户资料信息的列名进行排序
void CGTOWNLogInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNLogInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//使用关闭按钮
void CGTOWNLogInfo::OnBnClickedCancel()
{
