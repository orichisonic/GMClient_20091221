...
void CGTOWNTreasureBoxInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","TI_UI_tpgTreasureBox"));//宝箱开启信息
...
void CGTOWNTreasureBoxInfo::TreasureBox()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TreasureBox_Query))//宝箱信息
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
			...
			case 1:
				{
					...
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_TreasureBox_Query))//发送宝箱开启信息请求
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
LRESULT CGTOWNTreasureBoxInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::GT_UserInfo_Query_Resp://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::GT_TreasureBox_Query_Resp://宝箱开启信息的回应
		{
			if(pageInfo)//如果已经能够翻页
			{
