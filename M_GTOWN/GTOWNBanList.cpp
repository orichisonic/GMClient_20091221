...
IMPLEMENT_DYNAMIC(CGTOWNBanList, CDialog)
CGTOWNBanList::CGTOWNBanList(CWnd* pParent /*=NULL*/)
	: CDialog(CGTOWNBanList::IDD, pParent)
{
}

CGTOWNBanList::~CGTOWNBanList()
{
}

void CGTOWNBanList::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNBanList, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNBanList 消息处理程序

BOOL CGTOWNBanList::OnInitDialog()
{
...
void CGTOWNBanList::InitUI()
{
	...
	tbcResult.InsertItem(0,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_TpgList"));//停封账号列表
	//tbcResult.InsertItem(4,theApp.operGTOWN.ReadValue("MGTOWN","BU_UI_Tpgquary"));//已封停帐号查询
}

//处理快捷键响应
BOOL CGTOWNBanList::PreTranslateMessage(MSG* pMsg)
{
...
//自动调节大小
void CGTOWNBanList::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNBanList::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNBanList::OnBnClickedOk()
{
	try
	{
		CIni mIni;
...
void CGTOWNBanList::UserInfo()
{
	...
	{		
		...
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_BanUser_Query))//发送查询玩家资料信息的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
//翻页查询玩家资料信息
void CGTOWNBanList::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 查询所有被封停帐号列表信息
void CGTOWNBanList::AllBanUserList()
{
	try
	{				
		CIni mIni;
...
// 翻页进行查询//所有被封停的帐号
void CGTOWNBanList::OnCbnSelchangeCmbpage()
{
	try
	{		
		if(pageInfo)
...
// 单击用户列表保存当前行号
void CGTOWNBanList::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击用户列表切换界面
void CGTOWNBanList::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	try
	{
		if(listUser.GetItemCount() != 0)
...
void CGTOWNBanList::OnNMClickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CGTOWNBanList::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
//切换tabCtrl选项卡
void CGTOWNBanList::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	CIni mIni;
...
//封停或解封帐号
void CGTOWNBanList::OnBnClickedBtnban()
{
	try
	{
		CIni mIni;		
...
//重置信息
void CGTOWNBanList::OnBnClickedBtnreset()
{
	SetDlgItemText(IDC_EDITBANREASON,"");
...
//接受返回信息
LRESULT CGTOWNBanList::OnMessageReceive(WPARAM wParam,LPARAM lParam)
{
...
//显示玩家资料信息
void CGTOWNBanList::showUserInfo()
{
...
//显示封停帐号
void CGTOWNBanList::showBanPlayer()
{
...
//显示封停帐号列表
void CGTOWNBanList::showBanList()
{
...
//显示解封帐号
void CGTOWNBanList::showUnbindPlayer()
{
...
//查询帐号有没有封停
void CGTOWNBanList::showSearchPlayer()
{
...
//单击用户资料信息的列名进行排序
void CGTOWNBanList::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNBanList::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//使用关闭按钮
void CGTOWNBanList::OnBnClickedCancel()
{
