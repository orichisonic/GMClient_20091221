...
void CGTOWNItemInfo::InitUI()
{
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","TI_UI_tpgBagInfo"));//背包物品资料
	tbcResult.InsertItem(2,theApp.operGTOWN.ReadValue("MGTOWN","TI_UI_tpgCharcterItem"));//角色装备信息
	//tbcResult.InsertItem(3,theApp.operGTOWN.ReadValue("MGTOWN","TI_UI_tpgPetBag"));//宠物背包信息
}
...
void CGTOWNItemInfo::BagInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_UserPack_Query))//背包物品资料查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNItemInfo::CharacterItem()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_UserEquip_Query))//查询角色装备信息
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
void CGTOWNItemInfo::PetBag()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_PetPack_Query))//查询宠物背包信息
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
	...
	{	
		if(pageInfo)//如果可以翻页的话
		{
			...
			case 1:
				{
					...
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_UserPack_Query))//查询角色背包信息
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
			...
			case 3:
				{
					midlength=socketdata.EnBody();
					if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_PetPack_Query))//查询玩家交易日志
					{
						::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
						dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
					}
...
LRESULT CGTOWNItemInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
	...
	case CEnumCore::Message_Tag_ID::GT_UserInfo_Query_Resp://查询玩家资料信息的回应
		...
		break;
	case CEnumCore::Message_Tag_ID::GT_UserPack_Query_Resp://查询角色装备信息的回应
	case CEnumCore::Message_Tag_ID::GT_UserEquip_Query_Resp://查询角色装备信息的回应
	case CEnumCore::Message_Tag_ID::GT_PetPack_Query_Resp://查询宠物背包信息的回应
		{
			if(pageInfo)//如果已经能够翻页
			{
