...
void CGTOWNTransmit::OnBnClickedBtntransmit()
{
	...
	try
	{
		...
		if (AfxMessageBox("确认移动该玩家", MB_YESNO) == IDNO) 
		{
			return;
		}
		...
		_itoa(x,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::GT_X,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//操作员ID
		_itoa(y,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::GT_Y,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//操作员ID
		_itoa(z,strInt,10);
		socketdata.AddTFLV(CEnumCore::TagName::GT_Z,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//操作员ID
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_Transmit_Query))//发送重设玩家到出生状态的请求
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
