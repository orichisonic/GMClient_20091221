...
IMPLEMENT_DYNAMIC(CGTOWNBackMoney, CDialog)
CGTOWNBackMoney::CGTOWNBackMoney(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNBackMoney::IDD, pParent)
{
}

CGTOWNBackMoney::~CGTOWNBackMoney()
{
}

void CGTOWNBackMoney::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNBackMoney, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNBackMoney 消息处理程序

// 初始化对话框
BOOL CGTOWNBackMoney::OnInitDialog()
{
...
// 自动改变大小
void CGTOWNBackMoney::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNBackMoney::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//处理快捷键响应
BOOL CGTOWNBackMoney::PreTranslateMessage(MSG* pMsg)
{
...
// 查询玩家资料信息
void CGTOWNBackMoney::OnBnClickedSearch()
{
	try
	{
		CIni mIni;
...
//翻页查询玩家资料信息
void CGTOWNBackMoney::OnCbnSelchangeCmbupage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNBackMoney::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNBackMoney::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	/*if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNBackMoney::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
//道具购买消费记录查询
void CGTOWNBackMoney::ItemConsume()
{
	try
	{
		CIni mIni;
...
//切换类型查询道具购买消费记录
void CGTOWNBackMoney::OnCbnSelchangeCmbtype()
{
	try
	{
		CIni mIni;
...
//翻页进行查询
void CGTOWNBackMoney::OnCbnSelchangeCmbpage()
{
	try
	{	
...
// 接收返回信息
LRESULT CGTOWNBackMoney::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
// 显示角色资料信息
void CGTOWNBackMoney::ShowUserInfo()
{
...
//显示其它信息
void CGTOWNBackMoney::ShowOtherInfo()
{
...
//单击用户资料信息的列名进行排序
void CGTOWNBackMoney::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNBackMoney::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

//使用关闭按钮
void CGTOWNBackMoney::OnBnClickedCancel()
{
...
void CGTOWNBackMoney::OnBnClickedQuery()
{
	// TODO: 在此添加控件通知处理程序代码
	pageInfo = false;
	cmbPage.ResetContent();
...
void CGTOWNBackMoney::OnCbnSelchangeCmbuser()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
		...
		CString ItemName;
		GetDlgItemText(IDC_ITEMNAME,ItemName);
		socketdata.AddTFLV(CEnumCore::TagName::GT_ItemName,CEnumCore::TagFormat::TLV_STRING,strlen(ItemName),(unsigned char *)mIni.wcharToChar(ItemName));//道具名称
...
void CGTOWNBackMoney::OnNMDblclkListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	*pResult = 0;
	...
	if(listResult.GetItemCount() > 0)
	{
		...
		int iResultItem = listResult.SubItemHitTest(&lvinfo);//保存在玩家资料信息列表中选中的行号
		...
		if(iResultItem >= 0)
		{
			...
			CString SenderAccount = theApp.operGTOWN.GetValueByColName(&listResult, nItem, theApp.operGTOWN.ReadValue("MGTOWN","GT_SenderAccount"));//发送者帐号

			int iMoney=atoi(theApp.operGTOWN.GetValueByColName(&listResult, iResultItem, theApp.operGTOWN.ReadValue("MGTOWN","GT_MCash")));//M币
			int iTime=atoi(theApp.operGTOWN.GetValueByColName(&listResult, iResultItem, theApp.operGTOWN.ReadValue("MGTOWN","GT_TimeLimit")));//永久
			int iID=atoi(theApp.operGTOWN.GetValueByColName(&listResult, iResultItem, theApp.operGTOWN.ReadValue("MGTOWN","GT_ID")));//序号
			CString strItemName=theApp.operGTOWN.GetValueByColName(&listResult, iResultItem, theApp.operGTOWN.ReadValue("MGTOWN","GT_ItemName"));//道具名称
			if(iMoney!=0&&iTime==-1)
			{
				//MessageBox("可以退款");
				CString msg;
				msg.Format("确实要退款道具%s（金钱%i）至发送者帐号%s",strItemName,iMoney,SenderAccount);
				if (AfxMessageBox(msg, MB_YESNO) == IDNO) //提示是否要退款
				{
					return;
				}
				...
				socketdata.AddTFLV(CEnumCore::TagName::GT_ServerIP,CEnumCore::TagFormat::TLV_STRING,strlen(ServerIP),(unsigned char *)mIni.wcharToChar(ServerIP));//服务器IP
				socketdata.AddTFLV(CEnumCore::TagName::GT_ServerName,CEnumCore::TagFormat::TLV_STRING,strlen(ServerName),(unsigned char *)mIni.wcharToChar(ServerName));//服务器Name
				socketdata.AddTFLV(CEnumCore::TagName::GT_Account,CEnumCore::TagFormat::TLV_STRING,strlen(SenderAccount),(unsigned char *)mIni.wcharToChar(SenderAccount));//玩家帐号
				...
				socketdata.AddTFLV(CEnumCore::TagName::GT_MCash,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//M币

				_itoa(iID,strInt,10);
				socketdata.AddTFLV(CEnumCore::TagName::GT_ID,CEnumCore::TagFormat::TLV_INTEGER,sizeof(int),(unsigned char *)&strInt);//序号

				socketdata.AddTFLV(CEnumCore::TagName::GT_ItemName,CEnumCore::TagFormat::TLV_STRING,strlen(strItemName),(unsigned char *)mIni.wcharToChar(strItemName));//道具名称

				int midlength=socketdata.EnBody();
				if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_Back_Money))//退款
				{
					::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
					dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
				}
