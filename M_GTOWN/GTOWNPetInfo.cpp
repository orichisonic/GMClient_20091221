...
IMPLEMENT_DYNAMIC(CGTOWNPetInfo, CDialog)
CGTOWNPetInfo::CGTOWNPetInfo(CWnd* pParent /*=NULL*/)
: CDialog(CGTOWNPetInfo::IDD, pParent)
{
}

CGTOWNPetInfo::~CGTOWNPetInfo()
{
}

void CGTOWNPetInfo::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGTOWNPetInfo, CDialog)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_CLOSE()
...
// CGTOWNPetInfo 消息处理程序

//初始化对话框
BOOL CGTOWNPetInfo::OnInitDialog()
{
...
// 初始化显示字符
void CGTOWNPetInfo::InitUI()
{
	SetWindowText(theApp.operGTOWN.ReadValue("MGTOWN","PI_UI_PetInfo"));
	...
	tbcResult.InsertItem(1,theApp.operGTOWN.ReadValue("MGTOWN","PI_UI_tpgPetInfo"));//玩家宠物信息
}

//处理快捷键响应函数
BOOL CGTOWNPetInfo::PreTranslateMessage(MSG* pMsg)
{
...
//自动改变大小
void CGTOWNPetInfo::OnSize(UINT nType, int cx, int cy)
{
...
//设置背景颜色
HBRUSH CGTOWNPetInfo::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//查询玩家资料信息
void CGTOWNPetInfo::OnBnClickedSearch()
{
	// TODO: 在此添加控件通知处理程序代码
	try
	{
...
//切换翻页组合框
void CGTOWNPetInfo::OnCbnSelchangeCmbpage()
{
	try
	{	
...
// 单击用户列表保存当前行号
void CGTOWNPetInfo::OnNMClickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
// 双击玩家资料信息进行查询
void CGTOWNPetInfo::OnNMDblclkListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
// 切换选项卡进行查询
void CGTOWNPetInfo::OnTcnSelchangeTabresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	if(tbcResult.GetCurSel() == 0)
	{
...
void CGTOWNPetInfo::PetInfo()
{
	...
	{		
		...
		int midlength = socketdata.EnBody();
		if(theApp.operGTOWN.SendLogQuery((unsigned char *)socketdata.getDes(),midlength,CEnumCore::ServiceKey::GT_PetInfo_Query))//宠物信息查询
		{
			::EnableWindow(this->GetParent()->m_hWnd,false);//发送一个请求时，让整个界面都变成Disable的状态，避免客户端同时发出多个请求
			dlgStatus = theApp.operGTOWN.ReadIntValue("MGTOWN","Send");//当前的状态为发送查询请求
		}
...
// 接收返回信息
LRESULT CGTOWNPetInfo::OnMessageReceive(WPARAM wParam, LPARAM lParam)
{
...
//显示玩家信息
void CGTOWNPetInfo::ShowUserInfo()
{
...
//显示其他信息
void CGTOWNPetInfo::ShowOtherInfo()
{
	tbcResult.SetCurSel(1);
...
//单击用户资料信息的列名进行排序
void CGTOWNPetInfo::OnLvnColumnclickListuserinfo(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listUser,pNMHDR,pResult);
}

//单击结果信息的列名进行排序
void CGTOWNPetInfo::OnLvnColumnclickListresult(NMHDR *pNMHDR, LRESULT *pResult)
{
	theApp.operGTOWN.SortListInfo(&listResult,pNMHDR,pResult);
}

// 关闭对话框
void CGTOWNPetInfo::OnClose()
{
...
void CGTOWNPetInfo::OnBnClickedCancel()
{
